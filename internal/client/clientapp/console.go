...
func (console *clientConsole) send(message tea.Msg) {
	console.mu.Lock()
...
func (console *clientConsole) show(view consoleView)       { console.send(view) }
func (console *clientConsole) setLanguage(language string) { console.send(consoleLanguage(language)) }

func (console *clientConsole) finish(err error) error {
...
func (console *clientConsole) logWriter() io.Writer {
	if console.program == nil {
		return os.Stderr
	}
	return console
...
func (console *clientConsole) Write(data []byte) (int, error) {
...
func (model consoleModel) content(styled bool) string {
	...
	for _, item := range [][2]string{{"entry", model.view.entry}, {"invite", model.view.invite}} {
		...
		if styled {
			target := address
			if item[0] == "entry" {
				target = clientLaunchURL(address)
			}
