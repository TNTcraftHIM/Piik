...
func TestCaptureBorderPreferenceFollowsSourceAndProfile(t *testing.T) {
	for _, supported := range []bool{false, true} {
		t.Run(strconv.FormatBool(supported), func(t *testing.T) {
			...
			session := New(executable, nativecapture.Capabilities{VideoCapture: true, SoftwareVP8: true, HideCaptureBorder: supported}, false)
			...
			assertBorder := func(expected bool) {
				...
				if slices.Contains(arguments, "--hide-capture-border") != expected {
			...
			start := startShareRequest{Version: 9, ID: "request_start", Type: "start-share", ShareID: "share_123456",
				...
				HideCaptureBorder: true, EdgeCapacity: 1, Codec: "vp8",
			...
			for _, hide := range []bool{false, true} {
				handle(replaceShareSourceRequest{Version: 9, ID: "request_replace", Type: "replace-share-source", ShareID: start.ShareID,
					Source: nativecapture.CaptureTarget{Kind: "display", SourceID: "2", Title: "Other fixture"}, HideCaptureBorder: hide})
				assertBorder(hide && supported)
			}
