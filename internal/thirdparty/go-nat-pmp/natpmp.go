// Modified by Piik: context-aware requests; the original API delegates to them.
package natpmp
...
type caller interface {
	call(ctx context.Context, msg []byte, timeout time.Duration) (result []byte, err error)
}
...
// GetExternalAddressContext retrieves the external address until ctx ends.
func (n *Client) GetExternalAddressContext(ctx context.Context) (result *GetExternalAddressResult, err error) {
	msg := make([]byte, 2)
	msg[0] = 0 // Version 0
	msg[1] = 0 // OP Code 0
	response, err := n.rpc(ctx, msg, 12)
	if err != nil {
		return
	}
...
func (n *Client) AddPortMapping(protocol string, internalPort, requestedExternalPort int, lifetime int) (result *AddPortMappingResult, err error) {
	return n.AddPortMappingContext(context.Background(), protocol, internalPort, requestedExternalPort, lifetime)
}

// AddPortMappingContext creates, renews or deletes a mapping until ctx ends.
func (n *Client) AddPortMappingContext(ctx context.Context, protocol string, internalPort, requestedExternalPort int, lifetime int) (result *AddPortMappingResult, err error) {
	var opcode byte
	if protocol == "udp" {
		opcode = 1
...
func (n *Client) rpc(ctx context.Context, msg []byte, resultSize int) (result []byte, err error) {
	result, err = n.caller.call(ctx, msg, n.timeout)
	if err != nil {
		return
	}
