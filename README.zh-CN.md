<img src="./public/favicon.svg" width="64" height="64" alt="Screener 小电视标志">
...
Screener 是一个私密屏幕分享工具：一位房主，最多 20 位受邀好友。
朋友用浏览器就能观看，无须安装。
...
只有房间号？打开同一个 Screener 站点，选择 **加入房间**。
站点访问权限和房间密码设置仍然适用。

## 用浏览器分享

1. 打开你们使用的 Screener 站点，按提示输入站点密码。
...
2. Windows 打开 `screener-client.exe`，macOS 打开 `Screener Client.app`，
   Linux 运行 `./screener-client`。
...
| **连接站点** | 使用已有 Screener 站点，并在同一浏览器界面使用 Client 的采集与媒体能力。 |

Client 会打开系统浏览器。程序包包含 Go 应用和采集、临时链接辅助程序，
运行时无须安装 Node.js、npm 或 Go。Linux 原生采集使用系统的 Portal、PipeWire
和 GStreamer，平台细节见 [Client 指南](./cmd/screener-client/README.md)。
...
连接不上时，也可以查看 [Chromium WebRTC 常见问题](./cmd/screener-client/README.md#chromium-webrtc-connections)。

## 许可

Screener 自有代码采用 [MIT 许可](./LICENSE)，提供 [中文参考译文](./LICENSE.zh-CN.md)。
