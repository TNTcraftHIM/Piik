...
在支持的 Windows 版本上，选源器的 **程序 / 窗口** 和 **屏幕** 页提供 **显示采集边框** 开关，默认关闭。
选取或切换来源时可以修改，本次房主页面会记住选择。
Windows 10、系统策略或其他正在进行的采集可能让黄框保持可见，分享仍可正常进行。
Windows 10 的可选处理方法见[采集边框问题排查](../../docs/guide/troubleshooting.zh-CN.md#windows-分享时出现黄框)。
