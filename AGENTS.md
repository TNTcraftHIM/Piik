...
- Before dependent work starts, update the single actual owner among requirement, design, ADR or research, and update `docs/project-memory.md` or `docs/status.md` only when its snapshot materially changes. Do not create parallel owners or mirror source details across documents. Checkpoint the consistent truth in Git; chat is not durable truth.
