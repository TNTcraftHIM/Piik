...
- Every non-server endpoint stays within its advertised relay capacity and the deployment hard ceiling; an upstream receive edge does not consume this upload budget. The current production/release policy (21d5) permits at most two downstream physical media edges for non-server endpoints, while ordinary Browser Viewer relay is strictly limited to one until the re-encode/resource gates pass. Deployment may tighten these limits but never raise them; UA/visibility do not change capacity, and actual edge failure returns to the bounded route-reselection path. Dynamic evidence-driven capacity remains future work and must not infer capacity from device identity. Central media normally serves at most two roots; extra server-fed exceptional viewers require an explicit deployment egress/admission cap and must never create unbounded fanout.
