...
- Keep media distributed and automatic. Use centralized services for rooms, authentication, signaling, STUN, observability, and bounded SFU-root capacity. TURN is an optional authenticated built-in ICE compatibility transport for explicit exact-room canaries, disabled unless its complete tuple is configured; it is not a media topology.
- Let viewers join from a normal desktop or mobile browser without installing the sender application.
- STUN is required. In an explicitly allowlisted canary, each capable Web peer connection may start with STUN plus short-lived authenticated TURN/UDP under `iceTransportPolicy: "all"`; standard ICE preference keeps direct candidates first and may select relay without a second application transport state machine. A failed peer connection still follows one restart, one same-parent rebuild, one alternate peer, then an SFU/UDP virtual parent feeding at most two roots, followed by clear bounded failure. Non-allowlisted rooms and clients without the capability remain STUN-only. HTTPS/WSS remains TLS/TCP and is independent of this media-transport policy.
...
- A dedicated deployment subagent owns each staged production deployment and verification while root/mainline continues; root supplies the exact merged SHA, artifact hash, rollback target, and acceptance list, and the deployer must not expand scope.
- Immutable production releases must not share hard-linked dependencies or other permission-mutated files with a rollback release. Copy or reflink an independent tree, verify zero shared regular-file inodes, and only then change ownership or switch the release symlink.
