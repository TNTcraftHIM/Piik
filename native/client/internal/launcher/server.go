...
func Start(parent context.Context, staticDirectory, site, revision, localAccessPassword string) (*Server, error) {
	if parent == nil {
	...
	if metadata, err := os.Stat(indexPath); err != nil || !metadata.Mode().IsRegular() {
...
func (server *Server) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	case "/client", "/client/":
		...
		http.ServeFile(response, request, server.indexPath)
	default:
