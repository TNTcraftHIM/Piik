...
func TestDecodeRequestAcceptsOnlyTheCurrentShape(t *testing.T) {
	valid := []string{
		`{"version":6,"id":"request_hello","type":"hello"}`,
		`{"version":6,"id":"request_ping","type":"ping"}`,
	}
	...
	invalid := []string{
		...
		`{"version":6,"id":"short","type":"ping"}`,
		`{"version":6,"id":"request_ping","type":"ping","extra":true}`,
		`{"version":6,"id":"request_hello","type":"hello","nonce":"obsolete"}`,
		`{"version":6,"id":"request_ping","type":"ping"} trailing`,
	}
...
func TestDecodeRequestUsesTheSharedIdentifierBoundary(t *testing.T) {
	...
	payload := []byte(`{"version":6,"id":"` + identifier + `","type":"ping"}`)
	if _, err := decodeRequest(payload); err != nil {
		t.Fatalf("maximum identifier was rejected: %v", err)
	}
	tooLong := []byte(`{"version":6,"id":"` + identifier + `a","type":"ping"}`)
...
func TestEnvelopeAllowsAnExtensionToOwnItsStrictShape(t *testing.T) {
	payload := []byte(`{"version":6,"id":"request_extension","type":"extension","value":1}`)
