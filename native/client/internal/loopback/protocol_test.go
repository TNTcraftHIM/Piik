...
func TestDecodeRequestAcceptsOnlyTheCurrentShape(t *testing.T) {
	valid := []string{
		`{"version":7,"id":"request_hello","type":"hello"}`,
		`{"version":7,"id":"request_ping","type":"ping"}`,
	}
	...
	invalid := []string{
		...
		`{"version":7,"id":"short","type":"ping"}`,
		`{"version":7,"id":"request_ping","type":"ping","extra":true}`,
		`{"version":7,"id":"request_hello","type":"hello","nonce":"obsolete"}`,
		`{"version":7,"id":"request_ping","type":"ping"} trailing`,
	}
...
func TestDecodeRequestUsesTheSharedIdentifierBoundary(t *testing.T) {
	...
	payload := []byte(`{"version":7,"id":"` + identifier + `","type":"ping"}`)
	if _, err := decodeRequest(payload); err != nil {
		t.Fatalf("maximum identifier was rejected: %v", err)
	}
	tooLong := []byte(`{"version":7,"id":"` + identifier + `a","type":"ping"}`)
...
func TestEnvelopeAllowsAnExtensionToOwnItsStrictShape(t *testing.T) {
	payload := []byte(`{"version":7,"id":"request_extension","type":"extension","value":1}`)
