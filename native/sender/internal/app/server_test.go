...
func newAppRemoteServer(t *testing.T, terminal chan<- []byte) *httptest.Server {
	...
	mux.HandleFunc("/signal", func(response http.ResponseWriter, request *http.Request) {
		...
		authenticated := []byte(`{"type":"authenticated","protocol":"screener-v6","role":"host","peerId":"host-peer","roomExpiresAt":null,"maxViewers":3,"endpointMediaCopyCapacity":2,"hostOnline":true,"connectionId":null,"viewerPeerIds":[],"iceConfig":{"iceServers":[]},"viewerPolicy":"private-link","viewerAuthorizationGeneration":"viewer_generation_12345678"}`)
