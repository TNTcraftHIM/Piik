...
> 2026-08-22 truth audit hold: 自动 P2P-first 方向仍保留，但容量、SFU publication/subscription、TURN、fallback 与临时资源计数尚未完成整体裁决。本文件相关旧段落是审计输入，不是当前实现授权；详见 [TODO audit hold](./todo-audit-hold.md)。
...
Web 客户端只在当前 socket 已鉴权、分享在线且本端拥有 active 权威路由时运行应用层静默断链探测：五秒后发送带安全整数序号的 challenge，首个精确两秒 miss 只进入内部 suspect 并立即发送新序号 confirm，第二个精确两秒 miss 才废弃旧 socket 代次并直接建立新连接，不等待旧 closing handshake。response 必须匹配当前 socket 代次和序号；旧序号、旧 socket、已替换鉴权会话一律忽略。页面隐藏、恢复可见或定时器明显迟到时清除未决证据并重新等待完整五秒。服务端只向主动 opt-in 的请求 socket 回送相同序号，每 socket 每秒最多一次，不附带房间、密钥、候选或统计信息；因此 strict Native 不会收到未知消息。watchdog 本身不发送 `route-failed`、不修改路由权威，也不要求普通 P2P 或同 revision SFU 关闭健康媒体；现有 `peer-selected` TURN 授权仍严格绑定两端鉴权 session，任一 session 替换后按既有有界恢复路径处理。服务端 30 秒 ping/pong 默认值保持不变。
...
SFU publisher 将 64 kbps 映射到 `AudioPresets.musicStereo`、128 kbps 映射到 `musicHighQualityStereo`，256 kbps 使用显式 `maxBitrate`；三者都保留 `forceStereo: true` 与 `dtx: false`。LiveKit 2.22.0 先合并 room publish defaults，其中 `red: true`；本实现不传 `red` 覆盖值，保留既有 RED request。RED 与 Opus in-band FEC 是不同修复机制，保留前者不声称实际启用、替代或等价于后者。只改 SFU 会导致 route switch 行为分叉，因此 P2P/browser relay 与 SFU 必须同一 slice 落地。Viewer 继续在同一个持久 `MediaStream`/媒体元素中增删 screen video/audio track；不新建 audio element、Web Audio mix/resample graph、第二 audio representation 或独立音频时钟。默认每条 outbound edge 的 audio RTP payload ceiling 为 128 kbps，显式最高档为 256 kbps；Host2 对应最多 256/512 kbps，ordinary Browser Viewer1 对应最多 128/256 kbps（不含 RTP/SRTP/UDP/IP overhead）。stereo 仍可能增加编码与网络成本，且浏览器 relay 不保证跨 PeerConnection shared encode。未来 microphone voice 若进入范围，另建独立 source/track 与 AEC、降噪、gain、DTX 设计，不能作用于电影/游戏屏幕音频。详细证据与 Oopz 未知边界见 `docs/research/browser-screen-audio-quality.md`。
...
单个三窗确认的 C+B 事件仍是 edge-local，`severe` 与 `relative-fps` 都先且只尝试一个 active upstream 为 peer 或 SFU 的 Viewer；Host 排除，候选的 session/share/revision 必须当前，且 active children 加一不得超过服务端有效 relay capacity。服务端在现有 route prepare 上保留旧 active edge 与 connection generation；新 parent 在 active child IDs/map 外只建立一个 provisional `HostPeer`，answer/candidate 按 exact child/connection/revision 优先路由到该 PC。Viewer 以当前 prepare revision 的正 RTP、正 decoded frames 和 live video track 证明后，matching active revision 才把同一个 PC 原子移入 active map，再执行普通 child reconcile 并一次性提交拓扑、route 与候选 connection ID；provisional 禁止用新 connection ID 自动重试。prepare-ready 后的候选失败必须保留精确 revision/parent/connection identity 到 matching active 或 rollback；候选 parent 对同 revision 的重复 matching active 持续 fail-closed，不 promotion、不建新 PC，也不越权发送 `route-failed`，恢复只由 target Viewer probe 发起。其他 edge 的已鉴权 hard `route-failed` 先 abort soft probe 并让 target/candidate 回到 current authority，再立即进入原故障梯级。候选失败、超时、session/share 变化或无候选均回滚/保留旧边，不创建 SFU/TURN intent。同房同时最多一个 peer migration。若同一 Viewer parent peer/session/share 在五秒内由两个不同 child 分别确认任一类型，服务端把该 parent 的独立 `relayEligible` 暂停 30 秒，使拓扑有效容量为零但保留客户端广告值；后续 capacity 广播不能绕过。Host 不参与，parent 自身 upstream/观看不变；本 slice 只保证触发 child 的 MBB，其他 current children 暂沿既有有界重选，完整 sibling MBB 后续实现。到期惰性恢复，parent 新 session 或新分享立即恢复，stop/delete/disconnect 清理状态。该离散资格不命令 LiveKit/WebRTC 媒体层，也不把 RTT、CPU、码率、设备信息压成综合分数；窗口仍需真实质量矩阵校准。

上述 two-distinct-child corroboration 仅在未来独立验收并接受更高 relay-capacity tier 后有效；当前 Browser1 release 使其不可达，且不授权第二 child。
...
标准表示能力排在任何自定义双表示媒体实现之前。当前 Web P2P simulcast 因 receiver 不能选 RID 且跨 PeerConnection 无 shared-encode 契约而静态 no-go；当前 Web/LiveKit SVC 因 direct/peer 无独立选层、硬件不可证明且 pinned screen share 为 `L1T3` 而静态 no-go。Pinned LiveKit 的标准 exactly-two screen-share simulcast 已显式配置 `dynacast: false` 并通过 roots <=2、Host publication 上传边记账和 zero-descendant assignment/subscription 保留的 source preflight；Dynacast off 阻止 pinned SDK 按 subscribed-quality 更新切换 publisher encoding，不关闭 SFU per-subscriber BWE，也不保证实际收到 `HIGH`。下一 gate 在隔离整形下验证 zero-descendant SFU leaf 从 `HIGH` maximum 由 SFU BWE 独立降层和恢复；通过后不实现自定义媒体 selector。Root-with-children 的 suspect/确认/有界迁移是 default-on 前硬门。只有内建选择不满足门槛时才显式设置标准 subscriber quality；常驻成本违反硬件、游戏或上传预算时才测标准 sender 手动启停，全部标准路径失败后才进入 custom/native dual encode。当前 subscriber 未 attach `RemoteTrack`，因此不直接启用 `adaptiveStream`，但这不阻塞 SFU BWE leaf 候选。Cumulative Dynacast-on 只作为另一种 always-on 形态。本 SVC 静态审查不跑 browser。具体门槛由 ADR-0007 和[实时质量调研](./research/realtime-quality-adaptation.md)持有，任何方案都不得复用 PR #28 的 minimum-of-two 或绕过 stock WebRTC congestion control。
...
该 selected edge 不改写 active SFU assignment 或另建 route owner。未 answer 的
negotiating attempt 与同房其他 soft MBB 串行；answer 后 exact lease 成为绑定有效期间的
长期 active transport authority，不是房间级迁移锁，可与无关 soft MBB 共存。两种状态下
...
媒体后继续服务 sticky descendants。当前 Host 最多两条下游 physical media edges，普通 Browser Viewer/root/relay 最多一条；上游接收不计预算。无法挂到健康 root 的极端 viewers 只能
...
Host 与 Viewer 复用同一个连接详情开关；展开后且至少存在一组本地 metrics 时显示下载图标。点击才把当前内存快照投影为 `schemaVersion: 1` JSON 并创建本地 Blob 下载，随后回收 object URL；不上传、不写 browser storage。投影器只按固定字段白名单读取 route/direction、连接状态、protocol/candidate type、codec、分辨率/FPS/码率/loss/jitter/RTT、限制、encoder 和音视频播放/修复指标，所有缺失值写为 `null`。它不接受或输出 candidate address/port、stats/connection/media ID、peer/client/room/name、raw candidate/SDP、STUN/TURN URL、username、credential 或 token。Host 的 peer-relay 接收项必须先经过已有 `ViewerQualityEvidence` 脱敏转换，不能序列化 evidence guard 或原消息。

Host 与 relay Viewer 复用同一个纯 presentation reducer 保存远端 `ViewerQualityEvidence`。代际 identity 是 viewer、parent、connection 与 route revision；只有同 identity 且 sequence 严格递增才可承接旧字段，其他输入从新 evidence 重建。每个非空 metric 保存自己的本地观测时间并在五秒边界清空，页面 timer 总是指向最早边界；整块详情不因某个采样缺字段而卸载。`new`/`connecting`/`disconnected` 保留同 identity shell 但立即标为非 fresh，`failed`/`closed` 或 identity/session/share/route 变化清空。诊断导出只读取 fresh presentation；该层不增加 wire 字段、服务端状态或 `mediaGeneration`。
...
当前 release policy 的硬目标是 Host 任何时刻最多两条下游 physical media edges、普通 Browser Viewer 最多一条；普通 upstream receive 不计上传预算。系统按一个或两个观看者直接 P2P、更多观看者 peer-assisted、部署 SFU 中央兜底、最后 selected-edge TURN 的顺序自动分配和恢复，分享者与观看者不选择拓扑。默认媒体 transport 只使用 UDP；SFU 作为 virtual parent 正常只送一至两个 roots，roots 之后仍优先沿受限 peer descendants 扩散；多个无法挂到健康 root 的极端 viewers 只可使用另设上限的中央 edge。Production 已把普通 ICE 收敛为 STUN-only、删除旧全房 TURN wire，并为所有正常房间配置同一 controller 和 replacement selected-edge tuple；旧 `PEER_ICE_TURN_*` 配置启动失败。公网 SFU media、selected relay 与资源仍未验收，旧 release/coturn 仅作回滚。ADR-0004/0005 的媒体验收尚未完成。
...
当前部署实现一个由服务端确定的广度优先活动拓扑：Host 最多两个 child，普通 Browser Viewer 最多一个 child。服务器可在多个合格 parent 中选择，但每名 viewer 同时只有一个 active upstream，活动媒体图保持无环。新 viewer 加入或容量释放时只接纳等待节点；节点断开或当前 edge 失败时只重新分配失去 parent 的直接 subtree root，其 descendants 保持连接。客户端先关闭不再分配的旧 edge，再建立替代 edge；每次变更推进 route revision，stale signal/recovery 不得复活旧边。该规则不采集或组合 RTT、带宽、CPU、地理位置与质量评分，也不主动重平衡、多源接收或持续优化。没有 parent 时明确等待或拒绝，Host 不能创建第三条边，Browser Viewer 不能创建第二条边。

当前 Host2/Browser Viewer1 形成两条有界转发链；它满足发布资源边界，但不证明八人质量、更不证明 20 人能力。目标 20 人仍必须实测逐跳重编码、上行、延迟和故障半径，或使用受独立中央 egress cap 约束的 SFU 例外。未通过时保持当前默认八人，不增加 Host fanout 或绕过 Viewer 上限来凑容量。

所有边都继续使用标准 WebRTC。每名普通 Browser Viewer 播放收到的 remote `MediaStream`，并可把同一组 remote tracks 加入最多一个 child 的独立 `RTCPeerConnection`。该路径会逐跳解码并为 outbound sender 重编码；实验必须记录每层 encode time、CPU/GPU、`qualityLimitationReason`、帧率、分辨率和延迟，不能称为 zero-copy 或 shared encode。

部署参数 `MAX_PEER_RELAY_DOWNSTREAM_EDGES` 默认 `2`、只允许 `1/2`，是 release policy 的进一步收紧项：服务端有效 Host 上限为 `min(config, 2)`，普通 Browser Viewer 上限为 `min(config, 1)`。Host 的 active children、SFU publication、provisional/selected physical overlays 共用 Host 预算；Viewer 的 active child 与 provisional/selected physical overlays 共用 Viewer 预算，上游接收免费。旧/恶意 Viewer 广告 `2/3` 仍只能得到一条。Web assignment 执行层也用同一组当前 Host2/Viewer1 named constants 截断 child IDs，再创建 active、provisional 或 selected PC；这只提供客户端 defense-in-depth，服务端仍是路由权威。部署收紧到 `1` 时，SFU fallback 必须先把占用 Host 唯一 peer slot 的必要 branch 提升为 root，再建立占一条预算的 Host publication；root 预算独立且仍最多二。wire 不升级，`relay-capacity` 的 `0/1/2/3` 仅保留为未来 capability envelope；当前 Web client 广告 `1`。动态 evidence-driven capacity 仍须独立验收，不按 device/UA 分支，并由持续 sender bitrate、`availableOutgoingBitrate`、区间 encode time、`qualityLimitationReason`、loss/RTT 健康窗与 cooldown 共同限界。H.264、hardware encode 或 shared encode 可降低编码成本或同观感码率，但每个 child 的网络副本仍独立存在。

Peer-assisted 信令只接受一个严格对象：`resolution` 为 `720p | 1080p | 1440p`，`maxFramerate` 为 15 至 60 的整数，`maxBitrate` 为 2,000,000 至 12,000,000 的整数，`degradationPreference` 为 `maintain-resolution | balanced | maintain-framerate`，可选 `videoCodec` 为 `automatic | h264 | vp8`，可选 `screenAudioQuality` 为 `saver | music | very-high`，其余字段和越界值拒绝。服务端用房间内存 Map 保存完整设置，默认 1080p60、8 Mbps、`balanced`、`automatic`、`music`（128 kbps），旧的缺失音频字段也解析为该默认，不写 SQLite；停止分享时保留，房间被放弃或过期时删除。只有 peer-assisted host 可更新，鉴权快照和在线广播携带同一设置；本 wire 使用 `screener-v5`，普通 P2P 严格鉴权结构不变。SFU publisher 复用同一设置。
...
spike 复用现有 screen stream 和浏览器 codec 协商，当前普通 Web relay 不按桌面、Android 或 iOS UA 拆分 capacity；房间接入默认八名、显式允许 1 至 16，代表性资源/质量门禁仍按八名执行。捕获存在时沿同一标准 WebRTC 路径带上音频。移动真机表现作为非阻断兼容观察；Encoded Transform/DataChannel/WebCodecs 媒体、SVC/simulcast、多树、转码、codec ladder、综合评分、移动端后台 relay 和跨模式 SFU 迁移不在本次 spike 内实现。该范围限制不能解释为拒绝产品所需的自动兜底；验收与失败即放弃条件以 ADR-0004 为准，自动路由由后续独立 ADR 定义。
