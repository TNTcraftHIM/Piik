...
质量证据保持 edge-local，只能影响证据覆盖的 parent/child authority，不能形成全房综合分数。现有实现包含 Viewer 与 Host provisional-parent、媒体证明后 promotion、失败保留旧边、坏 relay corroboration 和 bounded quarantine 等可复用机制；整体模型统一 candidate eligibility、质量 owner 的独立关联证据、sibling migration、migration mutex、capacity reservation 与 fallback 交互。无论策略取值如何，prepare/commit/rollback 都保留 exact session/share/revision/connection/media-binding identity，ICE connected 不能替代 live track、正 RTP 和 decoded-frame progress。
