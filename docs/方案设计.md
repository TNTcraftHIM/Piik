...
5. 在已合入的 endpoint cap/accounting、`screener-v6` wire 与 SFU admission/lifecycle 边界上，按 ADR-0005 完成 route waves C-D：建立 TURN allocation admission，并把 deterministic allocation、child reparent 和 parent drain 收到一个 controller；每个 wave 独立可回滚。
