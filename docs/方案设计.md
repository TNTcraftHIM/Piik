...
`reconcileRoom` 在没有 pending operation 时先删除 disconnected childless participant，再找一个 waiting Viewer，或 parent offline/effective-capacity 不足、exact edge 已失效的 child。Candidate planning 拒绝 stale session、不可 source-reachable、成环、无 sender/overlap slot或 server resource 未准入的 tuple；对剩余 upstream 依次比较结果深度、剩余 sender slots、稳定加入顺序与 peer identity，并建立 direct/STUN、selected TURN、SFU 的有界顺序。每次只 reserve/prepare cursor 指向的 candidate。WebRTC 负责 ICE/DTLS/consent；exact candidate child 解码出第一帧新画面后发送一次 ready，controller 才 commit。失败先幂等清理并推进 cursor，operation 总 deadline 或耗尽进入 waiting/failure。没有上述离散事件时不改变拓扑。无环和房间 admission 已约束最大深度，不增加独立 depth hard cap。
