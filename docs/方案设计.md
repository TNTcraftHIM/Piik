...
状态：canonical `main` 包含精确 source implementation `064db16c1b6d25fd24b7057b7e2fca172002e3c4` 和严格 `screener-v12`；production 仍运行精确 revision `c4962f54443ad5f98bc65861195a3d9c74a48996`、release `c4962f5` 与严格 `screener-v11`，v12 尚未部署。源码已实现 22 字符 room-lived Viewer grant、`ROOM_NOT_FOUND`、产品诊断下载移除、完整短 URL 邀请 UI、收敛的房间密码控件与 exact-current audio stats。源码和生产均固定 Browser VP8，video 不设置 `contentHint`、audio 保留 `music`，且无 codec UI/state/wire 或分享生命周期分支；Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。Owner 已在生产实测现有 SFU fallback 成功取得画面。
...
- Canonical `main` 包含精确 source implementation `064db16c1b6d25fd24b7057b7e2fca172002e3c4`，使用严格单一 `screener-v12` wire，并已实现短 room-lived grant、明确 missing-room 结果、产品诊断下载移除、完整短 URL 邀请 UI 与 exact-current audio stats。Production 仍运行精确 deployed application/runtime revision `c4962f54443ad5f98bc65861195a3d9c74a48996`、release `c4962f5` 和严格 `screener-v11`，v12 尚未部署。源码与生产均固定 Browser VP8 且 video 不设置 `contentHint`，codec selector、quality-state/wire 字段与分享生命周期 codec 分支不存在。
