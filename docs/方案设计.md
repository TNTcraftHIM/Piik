...
每个 presence 条目包含 `role`、`peerId`、`displayName` 和服务端 active assignment 的精确 `upstream`：Host 为 `none`，Viewer 为 `none | peer(peerId) | sfu`。UI 始终展示名称加 peerId 尾部至少六字符短后缀；碰撞时逐字符有界延长至完整 peerId。卡片状态只依据本机连接或当前 Viewer 接收证据显示“已连接”，否则区分“线路已分配”和“连接中”；连接详情只显示 `P2P` 或 `SFU`，且仅在本机真实 selected relay 时附加 `TURN`。独立拓扑按钮从同一 snapshot 展示 Host、实际存在的 SFU 节点及精确上下游，不从客户端媒体快照推断。peerId 不因此成为授权身份或路由输入；名称、presence 与拓扑视图不改变授权、媒体或路由状态机。
