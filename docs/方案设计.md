...
拥有唯一 authoritative publication，所有 SFU-fed Viewer 订阅它。SFU-fed
Viewer 的容量广告或 ingress ready 不授予 relay eligibility；controller 只可预留
一个正式 sender slot，并以一条 exact provisional ordinary-child edge 作为证明事务。
该 child 同时证明 connected transport、live video、当前窗口正 RTP receive 与
decoded-frame progress，加上 exact sender ownership 和 controller admission 后才
atomic commit；失败、超时、暂停或 identity 变化销毁 candidate 并保持 leaf，后续
每个 child 仍分别证明自己的 exact edge。TURN 只替换授权 logical edge 或
Host-SFU ingress 的 transport，不是拓扑节点。现有 edge 优先保持 direct/STUN，
必要时使用 exact selected TURN，再按需要切换到 SFU；Host 无 direct first-level
path 时优先一条可经 TURN 的 Host-SFU publication。端点容量、SFU/TURN
ingress/egress、临时 overlap、media binding generation、server admission 和
bounded failure 在同一 reserve/proof/commit/rollback 事务中记账。
...
5. 在已合入的 endpoint cap/accounting、`screener-v6` wire、SFU lifecycle 与 TURN allocation admission 边界上，按 ADR-0005 完成 route controller convergence：把 deterministic allocation、child reparent 和 parent drain 收到一个 controller；该 wave 独立可回滚。
...
生产仍运行 `screener-v5`；当前 source 使用 `screener-v6`。不兼容 wire/schema 迁移须原子切换并保留迁移前备份。
...
服务端维护每个房间唯一的版本化 route owner，它独占 committed graph、pending target 和 transition lifecycle。拓扑 helper 只保存 participant metadata 并纯计算 candidate plan，不持有可回滚的第二份 active/pending graph。候选 parent 可以形成图，但 committed media graph 必须无环；Host 没有 upstream，每名 Viewer 同时只有零或一个 active upstream，且每个 active Viewer 必须沿 peer edges 到达 Host 或沿当前 authoritative Host publication 到达 source。加入、容量释放、节点离开、当前边失败和已接受的 path-health evidence 才能触发一次有界重选，未受影响分支保持 sticky。

Parent selection 使用硬过滤加字典序，不建立综合质量评分器。先拒绝 stale session、不可 source-reachable、成环、超深、无 sender/overlap slot、relay-ineligible、位于 failed-edge cooldown 或 server resource 未准入的候选；对剩余候选依次比较结果深度、分配后剩余稳态 sender slots、稳定加入顺序与 peer identity。controller 每次只 reserve/prepare 一个候选，使用该 `RTCPeerConnection` 自带的 ICE checklist/connectivity checks，再以 current-generation RTP、live track 与 decoded frame 验证；初次加入或 hard failure 采用达到 media-usable floor 的首个候选，soft-quality reparent 还需 quality owner 证明候选达到 recovery threshold 且优于仍健康旧 edge。失败先幂等清理候选再取下一个；受管 SFU room 完成删除证明前仍占 server capacity。应用不做 all-pairs ping、raw-IP/NAT/地理排名、周期探测或全房重平衡。一个 exact edge 的近期失败只进入该 edge 的有界 exclusion/cooldown，不污染其他路径。

所有非服务器端点共享一个 server-authoritative steady outbound media-copy capacity：唯一配置 `ENDPOINT_MEDIA_COPY_CAPACITY` 默认 `2`，只接受 `1`、`2` 或 `3`，不兼容旧键。peer child 或 Host publication 各占一份，upstream receive 免费，committed selected TURN 只替换同一份 edge transport；SFU subscriber egress 与 TURN allocation 使用独立 server admission。客户端 capability 只能收紧该值，不能按角色、浏览器、UA、设备或页面可见性建立另一套 policy。服务端通过 v6 `authenticated.endpointMediaCopyCapacity` 把最终值交给 Web 与 Native；Native 不从 `maxViewers` 推导发送预算，普通 admission、peer-assisted child assignment 和槽位分配共用该值。代码通过一个 shared accounting guard 执行，不在 Host、Viewer、active、recovery 或 migration 分支重复 magic number；client/server/native wire 随该能力原子升级。Selected TURN 使用显式无默认值的正 safe-integer deployment capacity；同一个进程内 ledger 对每条 exact peer-selected edge 和 Host-SFU ingress 在 credential 签发前 reserve `1`，并让 reserved、committed、draining authorization 持续计费至 exact logical release。`C=3` 满载时只阻止会产生第四份 endpoint media copy 的 token/grant；同一 edge 改用 TURN 不增加 endpoint copy，但仍需独立 TURN admission。

SFU admission 在应用单进程中只有一个注入实例。LiveKit credential tuple 完整时，必须同时显式配置私有 `LIVEKIT_API_URL`、正 safe integer `SFU_INGRESS_CAPACITY` 与 `SFU_EGRESS_CAPACITY`；容量没有产品默认值，也不从 Viewer 数、endpoint cap 或历史 root 数推导。专用 LiveKit 实例设置 `room.auto_create: false`。账本只保存全局已用计数和按 exact room/share/publication generation 索引的 `reserved | committed | draining` 条目：Host publication 计 ingress `1`，每个订阅计 egress `1`，candidate、committed 和未完成 drain 的旧 generation 同时计数。route controller 先 reserve，再通过 `RoomService.CreateRoom` 创建 exact-generation room，成功后才 prepare 和签发 token；media proof 后 commit candidate 并把旧 generation 转入 draining，失败、超时、失联、分享换代、停止或删房也只转入同一状态。只有 `RoomService.DeleteRoom` 成功且 follow-up lookup 证明 room 不存在，账本才扣除对应资源；auto-create 关闭后，旧 token 不能重建已删除 room。进程启动在接受流量前拒绝任何非 Screener room，删除全部遗留 Screener room 并确认 namespace 为空；关闭时先 drain 再遗忘计数。Host 信令离线期间若 exact room 中的 Host participant 仍存在就继续计费并保留健康媒体，participant 消失后在一个有界轮询周期内回收。稳态 reserve/commit/drain bookkeeping 仍是 Map lookup 与整数加减；只有 generation 创建、删除、启动清场和 Host 离线检查访问 LiveKit control plane。TURN allocation 使用独立的进程内 logical ledger；多应用进程部署必须先增加共享原子 admission/lifecycle owner。
...
质量事件按影响范围分工，而不是按谁先发现分工。Viewer 对自己 upstream 的 C 证据与 parent 对同一 edge 的 B 证据相关后，只触发该 child 的换父；仅当至少还有一条不同 sibling edge，且所有 current authoritative child edges 都各自确认 unusable，downstream evidence 才把 parent 标为 directional-ineligible 并退位排空。同一 edge 的 C+B、未知 evidence、未完成 proof 或任一 current child 的正进展都不能完成 parent corroboration。parent/session hard failure、修复失败的 relay-parent ingress，以及由服务端可验证 producer 确认的 endpoint-wide sender/resource failure 可独立 drain。Host source 走 publication repair/child migration，不走 relay-parent reparent。节点同时是 child 和 relay parent 时，先用 reparent 或 branch-preserving `replaceIngress` 修复其自身 ingress、停止新 child；新 ingress 恢复下游媒体时保留子树，否则由同一 controller 合并下游 drain。C+B 不产生第二个 controller 或第二个 revision；pause、evidence carry 和 exact-edge invalidation 共用同一 generation fence。
