...
启动失败，且绝不是 selected-edge tuple 的 alias。Source candidate 已在同一 coherent
change 中加入完整 default-off `SELECTED_EDGE_TURN_*` tuple、controller consumer 与
Host/ViewerRelay/Viewer rebuild；部分配置、非法 UDP TURN URI、弱或复用 secret、越界
TTL 都启动失败。该 candidate 尚未部署或通过真实 relay-media 验收。
...
配置。pending 超时、失败或代次变化清理 attempt；answer 后 attempt 变为已使用并仅为
该 edge 保留信令 owner，直到失败或 generation/session/topology/room cleanup。普通 PC、其他 edge、
...
presence 仍报告 `sfu`，本地 ICE path 才报告 TURN transport。签发前按当前 assignment
重新计算 parent 下游预算，Host 的 SFU publication 也占一边；Host 两边或 ViewerRelay
一边已满且 target 不是现有 child 时 fail closed，不替换无关 child。
...
Source candidate 的可执行梯级是 `direct/peer UDP -> SFU ICE/UDP -> selected-edge
TURN -> failure`，production 仍停在 SFU 后的 bounded failback。Viewer 的
...
6. 默认关闭的 selected-edge TURN tuple、revision-bound one-use grant 与 parent/child rebuild 已在 source candidate 闭环；其后再用有界网络矩阵验收 ordinary STUN-only、SFU/UDP、单条 forced relay、资源成本和明确失败，coturn/production rollout 保持独立验收边界。保留现有 benchmark，但统一性能对比不是功能落地前置。
...
产品硬目标是任一非服务器节点任何时刻最多两条下游媒体边。系统按一个或两个观看者直接 P2P、更多观看者 peer-assisted、部署 SFU 中央兜底、最后 selected-edge TURN 的顺序自动分配和恢复，分享者与观看者不选择拓扑。默认媒体 transport 只使用 UDP；SFU 作为 virtual parent 正常只送一至两个 roots，roots 之后仍优先沿受限 peer descendants 扩散；多个无法挂到健康 root 的极端 viewers 只可使用另设上限的中央 edge。Production 已把普通 ICE 收敛为 STUN-only、删除旧全房 TURN wire并只为 room `1` 开启 failure-only controller；source 也已删除失败的 participant-wide built-in code/wire，旧 `PEER_ICE_TURN_*` 配置启动失败，replacement selected-edge 已完成 source candidate 但尚未部署。exact-room 公网 SFU media、selected relay 与资源仍未验收，旧 release/coturn 仅作回滚。ADR-0004/0005 的 broad-rollout 验收尚未完成。
