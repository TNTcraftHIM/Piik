...
房间只有一种易失模型。进程启动时建立 `1000..9999` 的空闲码池；创建房间可携带一个严格四位 `preferredRoomId`，若它仍在空闲池则以同一个 swap-remove 取走，否则用加密随机索引从当前空闲集合分配，释放时放回。偏好请求不预占、不替换现存房间，也不改变新房凭据生成。这样在 9000 个号码内没有碰撞重试、递增身份或数据库序列。`MAX_ROOMS` 默认 1000 且不得超过 9000。
