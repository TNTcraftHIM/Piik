...
状态：production Browser runtime 为 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`；canonical main 包含该 runtime 和其上的当前 docs-only truth checkpoints。当前 Browser 合同包括 live-media mutation、Viewer presentation/diagnostics、无枚举房间准入和质量选择，并已通过原子部署 postflight；精确 deployed behavior 只由 deployment 文档记录。
...
下表描述 accepted Browser 的单一 `screener-v10` wire；server 与 Browser assets 必须作为一个完整合同原子发布，v9 和更早客户端在取得房间 authority 前拒绝，不存在旧 parser、writer、translator 或中间协议子集：
...
accepted 的唯一 wire 是 `screener-v10`，不做版本协商、v9 parser、双写或 translator。任何 v9 或更早页面及 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
浏览器可返回更低设置，界面以 `track.getSettings()` 显示实际捕获结果。推荐区始终只有表中的三档并默认选择中档 `1080p30`；高级分辨率额外提供 `480p`（854x480）、720p、1080p 与 1440p，FPS 继续独立接受 15 至 60 整数，码率继续独立接受 2 至 12 Mbps，因此不建立第四个推荐档或 `480p` preset ID。视频 track 使用 `contentHint = "motion"`，返回的音频 track 使用 `contentHint = "music"`；二者都是内容提示而不是 codec 或质量档位。三档推荐质量与折叠高级区初始值统一采用 `degradationPreference = "balanced"` 与 `VP8`，并允许显式选择 clarity-first 或 fluid。“分享高级设置”在开始分享前提供 `自动 | H.264 | VP8` 作为 H.264 对照测试入口，并提供离散的 `64 kbps 省流 | 128 kbps 音乐默认 | 256 kbps 极高音质` 屏幕音频 sender ceiling。音频 ceiling 可在直播中串行更新；视频 codec 在分享开始后固定，直播和暂停期间均禁用 selector，改变 codec 需要停止并重新开始分享。它们只是 ceiling 和浏览器偏好，不保证实际分辨率、帧率、码率、声道或硬编。
...
Web 视频端不做视频 SDP munging。当前默认 `VP8`；临时测试期内，`automatic` 以空 `setCodecPreferences([])` 恢复 P2P 浏览器默认且不覆盖 LiveKit 默认 codec，显式 H.264/VP8 在本轮分享的首个 offer/publication 前应用。P2P 与 browser relay 始终保留完整 browser codec/repair 回退列表，SFU 始终保持 `backupCodec: false`。分享开始后 codec preference 固定；现有 peer 不因暂停而重协商，SFU 不因暂停而 republish，v10 wire 不包含 codec generation、preparation、proof、rollback 或 replacement 消息。普通 Pause/Resume 只以当前 Host session 和 `shareGeneration` 更新一个 paused boolean、切换同一批 capture tracks并唤醒普通 route reconciliation。实际 outbound codec/profile、encoder implementation 与 stats 是结果真相；浏览器支持 H.264 不证明命中硬编。codec transaction 清理后立即定位并修复 H.264 根因，通过物理 direct、browser relay 与 SFU 验证后只选择 VP8 或 H.264 中一个作为固定 Browser codec，并在任一结论下删除 selector 与 codec-choice 产品功能。AV1 仍不得仅凭压缩率推断默认启用。唯一获准的 Web SDP 变换是下述 Viewer answer 的 Opus stereo receive preference，不得借此扩展视频码率、codec 或通用 munging。
...
直播中切换质量不重新打开选源器：对当前视频 track 调用 `applyConstraints()`，再更新所有现有 sender 的参数；该 mutation 不包含 codec，新加入、重建的 relay sender 和启用的 SFU publisher 使用本轮分享开始时固定的 codec 与最新其他权威设置。失败时保留当前分享并给出状态，不以重建整个房间补偿。临时暂停禁用当前 capture stream 的全部音视频 tracks；Host 通过当前 session 与 `shareGeneration` 发送一个 paused boolean，服务端同步 `host-status`、route pause 与 `authenticated` 快照，Viewer 在舞台显示“分享者已暂停”。Resume 使用同一状态更新并重新启用同一批 tracks，不另设 authorization、source acknowledgement 或 proof。晚加入和信令重连从快照恢复，恢复、停止、新分享或会话替换清除旧状态；不得把 track mute、黑帧或网络停顿推断成主动暂停。房间与连接保持不变。
...
- accepted next source contract 的单一 `screener-v10` 信令 literal、v9 及更早客户端 mismatch 的一次性刷新终态，以及协议非法 JSON、未知消息、错误方向、跨房间目标和 payload 限制；v10 不含 parent-edge quality proof 或 codec transaction/resume handshake，只保留 shareGeneration-fenced paused boolean、`ROOM_ACCESS_DENIED`、typed Viewer route failure 与 Host-only 按需脱敏 route snapshot，不保留旧 parser、writer 或别名；`qualityProfileIdSchema` 保持三档不变，只有 `qualityResolutionSchema` 包含 `480p`，`DEFAULT_QUALITY_SETTINGS` 为 1080p30、VP8；
...
- Production 当前运行 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`，因此仍是 automatic 默认、paused codec transaction 与 Resume source-ack/proof 行为；该 release 的 deployment postflight 已通过，回滚边界为 `/opt/screener/backups/39fcf93-pre-v9-20260824T004257Z`。本设计正文的 VP8 默认、pre-share-only selector、简单 Pause/Resume 与 `screener-v10` 是下一次原子实现/部署边界，尚未进入当前 source 或 production；v10 上线时拒绝 v9，不保留 alias 或兼容 parser。
