...
状态：production Browser runtime 为 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`；canonical main 包含该 runtime 和其上的 docs-only postflight truth。当前 Browser 合同包括 live-media mutation、Viewer presentation/diagnostics、无枚举房间准入和质量选择，并已通过原子部署 postflight；精确 deployed behavior 只由 deployment 文档记录。
...
- Production 当前运行 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`。Live audio ceiling、paused codec transaction、first-frame Viewer presentation、typed failure、按需 route timing、无枚举房间准入、1080p30 默认值和 advanced 480p 已随同一 wire 原子部署；该 release 的 deployment postflight 已通过，回滚边界为 `/opt/screener/backups/39fcf93-pre-v9-20260824T004257Z`。
