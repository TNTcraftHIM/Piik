...
当前 Windows Native sender 以所选游戏 PID 与进程创建时间为本地边界，使用 WASAPI process loopback 的 include-target-process-tree 捕获游戏及其子进程。目标进程无渲染流、退出或捕获失败时显示无声/未知，不回退为整机音频，也不上传 PID、路径、窗口标题、设备标识或原始音频。该 source-only 路径当前只支持 Windows 11；Windows 10 game-only 音频保持明确不支持，打包与其他媒体 route 仍待完成。
