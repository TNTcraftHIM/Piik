...
- 一名在线分享者、最多 `20` 名 authenticated Viewer；房间 admission 与 endpoint/SFU/TURN capacity 分离，当前 production 默认八名且 source 只接受 1 至 16 是待本轮发布替换的部署事实；
...
5. 在已合入的 endpoint cap/accounting、`screener-v6` wire、SFU lifecycle 与 TURN allocation admission 边界上，按 ADR-0005 完成 route controller convergence：一个 event-driven reconcile 和一个 room-serial candidate 统一处理 deterministic allocation、child reparent 和 relay drain；该 wave 独立可回滚。
...
服务端维护每个房间唯一的版本化 route owner，它独占 committed graph、一个 pending child operation 和 transition lifecycle。拓扑 helper 只保存 participant metadata 并纯计算 candidate list，不持有第二份 graph。operation 内聚 list/cursor、一个 current candidate/预留和一个总 deadline。committed media graph 必须无环且 source-reachable；Host 没有 upstream，每名 Viewer 同时只有零或一个 active upstream。加入、容量变化、节点离开、当前 edge `failed/closed`，以及非暂停时的 decoded-frame stall 都只唤醒同一个 `reconcileRoom(roomId)`；未受影响分支保持 sticky。

`reconcileRoom` 在没有 pending operation 时先删除 disconnected childless participant，再找一个 waiting Viewer，或 parent offline/effective-capacity 不足、exact edge 已失效的 child。Candidate planning 拒绝 stale session、不可 source-reachable、成环、无 sender/overlap slot或 server resource 未准入的 tuple；由 failed edge 启动时还把当前 `parent + transport` tuple 预先记为已尝试。对剩余 upstream 依次比较结果深度、剩余 sender slots、稳定加入顺序与 peer identity，先建立其他 direct/STUN，再建立 selected TURN 与 SFU 的有界顺序；同一 parent 的不同 transport 仍是不同 tuple。每次只 reserve/prepare cursor 指向的 candidate。WebRTC 负责 ICE/DTLS/consent；exact candidate child 解码出第一帧新画面后发送一次 ready，controller 才 commit。失败先幂等清理并推进 cursor，operation 总 deadline 或耗尽进入 waiting/failure。新的外部事实才开启新 operation 并重新计算候选。无环和房间 admission 已约束最大深度，不增加独立 depth hard cap。
...
relay 自身 ingress 失效时，reconcile 把它作为 child 换父并保留 subtree。endpoint disconnect 或 effective capacity 降低时，只保留稳定顺序内可容纳的 children，并把 overflow children 逐个送回同一 reconcile；offline childless participant 自然删除。权威 pause abort pending operation、保留 active graph、让新参与者 waiting，并抑制 decoded-frame stall；resume 重新 reconcile。A/B/C、码率、FPS、模糊和 SFU layer 只用于诊断。
