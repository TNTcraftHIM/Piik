...
单个三窗确认的 C+B 事件仍是 edge-local，`severe` 与 `relative-fps` 都先且只尝试一个 ordinary peer-upstream Viewer；v1 排除 Host 与 SFU root，但继续枚举后续合格 Viewer，二者的 provisional-child 支持留待后续。服务端在现有 route prepare 上保留旧 active edge 与 connection generation；新 parent 只建立一个 provisional edge，Viewer 以当前 prepare revision 的正 RTP、正 decoded frames 和 live video track 证明后，服务端才一次性提交拓扑、route 与候选 connection ID。prepare-ready 后的候选失败必须保留精确 revision/parent/connection identity 到 matching active 或 rollback；matching active 先到时立即报告 active failure，不能 promotion 已失败 peer。其他 edge 的已鉴权 hard `route-failed` 先 abort soft probe 并让 target/candidate 回到 current authority，再立即进入原故障梯级。候选失败、超时或无候选均回滚/保留旧边，不创建 SFU/TURN intent。同房同时最多一个 peer migration。若同一 Viewer parent peer/session/share 在五秒内由两个不同 child 分别确认任一类型，服务端把该 parent 的独立 `relayEligible` 暂停 30 秒，使拓扑有效容量为零但保留客户端广告值；后续 capacity 广播不能绕过。Host 不参与，parent 自身 upstream/观看不变；本 slice 只保证触发 child 的 MBB，其他 current children 暂沿既有有界重选，完整 sibling MBB 后续实现。到期惰性恢复，parent 新 session 或新分享立即恢复，stop/delete/disconnect 清理状态。该离散资格不命令 LiveKit/WebRTC 媒体层，也不把 RTT、CPU、码率、设备信息压成综合分数；窗口仍需真实质量矩阵校准。
