...
产品面向一名分享者和小范围可信朋友的低延迟游戏画面分享。观看端使用普通桌面或手机浏览器；每条普通媒体边独立 ICE 并优先 direct/peer UDP。所有非服务器端点共享一个服务端权威稳态 outbound media-copy capacity，默认 `2` 且只允许静态配置 `1`、`2` 或 `3`；peer child 或 Host publication 各消耗一份，upstream receive 免费，selected TURN 只替换同一份 edge 的 transport，角色、浏览器、UA、设备和页面可见性不改变该值。活动媒体图无环，每名 Viewer 同时至多一个 active upstream。SFU subscriber egress、TURN allocation、temporary overlap 和 exact fallback sequence 由 ADR-0005 的整体模型统一定义。
...
所有非服务器端点共享一个 server-authoritative steady outbound media-copy capacity：默认 `2`，只允许静态配置 `1`、`2` 或 `3`。peer child 或 Host publication 各占一份，upstream receive 免费，committed selected TURN 只替换同一份 edge transport；SFU subscriber egress 与 TURN allocation 使用独立 server admission。客户端 capability 只能收紧该值，不能按角色、浏览器、UA、设备或页面可见性建立另一套 policy。代码通过一个 named config 和 shared guard 执行，不在 Host、Viewer、active、recovery 或 migration 分支重复 magic number。
