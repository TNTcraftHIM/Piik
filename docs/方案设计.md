...
状态：exact source commit `d543f38aacad3df5ef65fde1055cc8e733972afe` 已实现单一 `screener-v9` Browser 合同，包括 live-media mutation、Viewer presentation/diagnostics、无枚举房间准入和质量选择。Production 仍是 exact `8f5b3f192ddd010ca01c969008e512191312736a`、release `8f5b3f1`、wire `screener-v8`，尚未部署 v9；精确 deployed behavior 只由 deployment 文档记录。
...
下表描述当前 source 的单一 `screener-v9` wire；server 与 Browser assets 必须作为一个完整合同原子发布，不存在 v8 parser、writer、translator 或中间协议子集：
...
| host -> server -> host | `request-route-diagnostic` / `route-diagnostic-snapshot` | 仅在 authenticated Host 点击下载诊断时请求并返回一份当前快照；不连续推送、不参与 route authority |
| host/viewer -> server | codec transition apply/proof | 精确绑定 share/codec generation、`resumeAttempt` 与 frozen media binding，独立于 `route-ready` 和 Viewer quality evidence |

Zod 在服务边界拒绝未知类型、超长字符串、非法 SDP/candidate 结构和越权目标。服务端根据已认证会话决定路由：观看者不能给另一名观看者发信令，分享者也不能向本房间以外的 peerId 发送。

当前唯一 source wire 是 `screener-v9`，不做版本协商、旧 parser、双写或 translator。任何旧页面或 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
- 当前 source 的单一 `screener-v9` 信令 literal、任意旧客户端 mismatch 的一次性刷新终态，以及协议非法 JSON、未知消息、错误方向、跨房间目标和 payload 限制；v9 不含无消费者的 parent-edge quality proof，包含 paused codec transaction proof、`ROOM_ACCESS_DENIED`、typed Viewer route failure 与 Host-only 按需脱敏 route snapshot，不保留 v8 parser、writer 或别名；`qualityProfileIdSchema` 保持三档不变，只有 `qualityResolutionSchema` 包含 `480p`，`DEFAULT_QUALITY_SETTINGS` 为 1080p30；
...
- Exact source `d543f38aacad3df5ef65fde1055cc8e733972afe` 是一个完整的 Browser v9 release candidate；live audio ceiling、paused codec transaction、first-frame Viewer presentation、typed failure、按需 route timing、无枚举房间准入、1080p30 默认值和 advanced 480p 必须随单一 `screener-v9` wire 一起原子部署，不发布协议子集。
- Production 当前仍运行 exact `8f5b3f192ddd010ca01c969008e512191312736a`、release `8f5b3f1`、wire `screener-v8`。Git commit、部署备份和原子 symlink cutover 承担回滚；生产切换前不得把 source v9 行为写成已部署。
- H.264/模糊与 Host 后台性能继续按受控根因证据诊断；标准 ICE/SFU 继续按真实异构网络验收，不增加 guessed candidate、NAT classifier、TCP probe 或 quality-driven reparent。
- 当前发布面只包含 Web Host、Viewer 和 browser relay。Native sender、capture helper、shared-encode executable 与两个分发包仍由各自后续决策和物理证据驱动，不参与 Browser v9 部署。
