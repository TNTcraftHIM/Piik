...
浏览器可返回更低设置，界面以 `track.getSettings()` 显示实际结果。视频 track 保留 `contentHint = "motion"`；sender 尝试设置 `maxBitrate`、`maxFramerate` 与 `degradationPreference = "balanced"`。`balanced` 只允许浏览器在帧率和分辨率之间权衡，不保证清晰度优先；具体 Chromium、Firefox 与 Safari 行为仍需实测。不支持 sender 参数时只记录警告。
...
直播中切换质量档位不重新打开选源器：对当前视频 track 调用 `applyConstraints()`，再更新所有现有 sender 的参数；新加入或重建的 sender 使用当前档位。失败时保留当前分享并给出状态，不以重建整个房间补偿。临时暂停只禁用当前视频 track，使远端收到黑画面；音频、房间和连接保持不变，恢复时重新启用同一 track。
...
用户可见的非直播状态只使用“已停止分享”（分享端已结束当前发布）和“等待开始分享”（观看端等待发布）。房间寿命、媒体代次、换父和恢复等内部状态不得作为额外产品文案暴露。
...
新的产品硬目标是分享端任何时刻最多两条下游媒体边。首选顺序为一个或两个观看者直接 P2P，更多观看者尝试 peer-assisted，部署者仍可显式选择可选 SFU；这不是静默的房间内 P2P/SFU 迁移。
...
Peer-assisted 信令只接受 `1080p60`、`1080p30`、`720p30` 三个 `qualityProfileId`。服务端用受房间容量限制的内存 Map 保存当前值，默认 `1080p60`，不写入 SQLite；停止分享时保留，房间被放弃或过期时删除。peer-assisted 鉴权快照携带当前值，host 鉴权后先重报本地选择再创建已分配的 child，直播中换档时通知所有在线 viewers，后加入者直接取得最新快照。普通 P2P 的严格鉴权 wire 保持原样，host 或 viewer 在该模式发送档位同步消息都会被拒绝。

Viewer 在处理 peer-assisted 鉴权快照时先更新 `ViewerRelay` 的 desired profile，再应用拓扑分配；同一个串行队列协调 remote stream、当前 child 更新和以后重建的 child，因此它们读取同一最新目标。每个 `HostPeer` 也把初始 sender 配置、`replaceStream()` 和 `updateProfile()` 放进同一 sender-mutation queue；`desiredProfile` 在入队前同步更新，已排队操作执行时读取最新值，使快速连续换档保持 last-wins，而不增加版本号或确认协议。这里同步的是浏览器 sender 的目标上限，不是实际分辨率、帧率、码率或 shared encoding 的性能保证。

spike 复用现有 screen stream 和浏览器 codec 协商，覆盖当前桌面 Chrome/Edge relay、按测试顺序位于链尾的当前 Android Chrome 与 iOS Safari leaf、最多八名 viewer；捕获存在时沿同一标准 WebRTC 路径带上音频。Encoded Transform/DataChannel/WebCodecs 媒体、SVC/simulcast、多树、转码、codec ladder、综合评分、移动端后台 relay 和自动 SFU 迁移均不实现。验收与失败即放弃条件以 ADR-0004 为准；成功也只能触发新的生产 ADR。
