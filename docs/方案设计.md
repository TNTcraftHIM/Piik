...
质量自适应采用 ADR-0007 的路径隔离单表示，而不是全房间按最差链路降档。Direct/peer 保留每条 PeerConnection 的 stock WebRTC GCC。Browser Host 的唯一 SFU publication 发布一层 `HIGH` VP8，不启用 `LOW`、simulcast、Dynacast、SVC 或应用媒体层 selector。Host 到 SFU 的 stock WebRTC 拥塞控制决定该共享 ingress 的实际码率、分辨率和帧率；LiveKit 为每个 Viewer 独立发送下行，但不能从单一编码表示生成另一档画质。应用证据只用于诊断，弱下行不命令 Host 降档或改变拓扑。任何未来 Browser 双表示候选都必须以新决策证明额外表示不挤占 `HIGH` 的上传、编码和游戏预算。
...
标准表示能力排在任何自定义双表示媒体实现之前。当前 Web P2P simulcast 缺少 receiver 选 RID 与跨 PeerConnection shared-encode 契约；当前 Web/LiveKit SVC 也缺少 direct/peer 独立选层和可移植硬件保证。Browser SFU 因此只发布单层 `HIGH`；具体表示门槛由 ADR-0007 和[实时质量调研](./research/realtime-quality-adaptation.md)持有，不得绕过 stock WebRTC congestion control。
