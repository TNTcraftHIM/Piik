...
每个 presence 条目包含 `role`、`peerId`、`displayName` 和服务端 active assignment 的精确 `upstream`：Host 为 `none`，Viewer 为 `none | peer(peerId) | sfu`。UI 始终展示名称加 peerId 尾部至少六字符短后缀；碰撞时逐字符有界延长至完整 peerId。卡片状态只依据本机连接或当前 Viewer 接收证据显示“已连接”，否则保持中性的“线路分配中”或具体连接状态；active assignment 本身不是连接成功证据。连接详情按 authoritative assignment/controller route 显示媒体徽标，即使媒体仍在连接：`P2P` 为绿色图标，`SFU fallback` 为黄色图标，且仅在本机真实 selected relay 时另附 `TURN`。独立拓扑按钮从同一 snapshot 展示 Host、实际存在的 SFU 节点及精确上下游，不从客户端媒体快照推断。peerId 不因此成为授权身份或路由输入；名称、presence 与拓扑视图不改变授权、媒体或路由状态机。
...
Host 新 share generation 的首次 `authenticate` 可选带 `debugP2pOnly: true`。服务端在 participant 进入 router 前把它绑定为当前进程内的 room/share-generation 状态；Viewer schema 严格拒绝该字段。同 generation 信令重连保持权威状态，不能借重连中途切换；`stop-sharing`、新 generation、room 删除和进程重启清除。新进程可从 Host 不可变认证 identity 重新接受该 generation 的请求，`authenticated.debugP2pOnly` 回显服务端实际状态，避免 UI 与路由状态分叉。启用时 router 仍执行 STUN-only direct、ICE restart/rebuild 与 alternate peer，但不创建 SFU prepare/config/token/publication、不发 standby URL，也不签发 `peer-selected` 或 `host-sfu-ingress` TURN；路径耗尽沿现有 `PEER_NOT_FOUND` 有界失败。它不持久化、不改变默认自动路由，也不充当 NAT 类型判断器；Hard NAT、TCP 与生日探测边界继续以[低服务器成本媒体路由](./research/low-server-media-routes.md)为准。
...
两端后续默认显示紧凑在线 roster；Host 与 Viewer 的本机显示名在界面中统一标为“昵称”，默认只读，铅笔按钮紧贴昵称并只切换当前控件的输入、保存和取消，展示仍为名称加 room-scoped `peerId` 短后缀。一个不持久化的“显示连接详情”复选框按页展开信令、ICE/TURN、浏览器实际暴露的本地/远端候选地址、codec、RTT、RTP 丢包率、编解码耗时及逐边统计；真实连接失败、音频缺失、持续画质受限和需要用户动作的告警始终可见。显示名/roster 不进入 access credential 或 SQLite。
