...
Web 视频端不做视频 SDP munging。`automatic` 以空 `setCodecPreferences([])` 恢复 P2P 浏览器默认且不覆盖 LiveKit 默认 codec；显式 H.264/VP8 在首个 offer/publication 前应用，P2P 与 browser relay 仍保留完整 browser codec/repair 回退列表，SFU 始终保持 `backupCodec: false`。暂停时的 codec 切换使用单调 codec-negotiation generation，并和 route mutation 串行：冻结 exact current route/media binding target，新 join 按 pause 语义等待、leave 从 target 移除；P2P/relay 在暂停中以独立总 deadline 显式重新 offer/answer，SFU 以新 publication generation republish。Preparation 成功后只停止 preparation deadline；同一个 room-serial codec mutation owner 保留一个内存有界的 prepared generation，暂停期间无 timer并排斥 route mutation，绝不擅自恢复用户主动暂停的分享。Host 的正常 Resume 动作先在该 owner 内原子重验 frozen bindings，由服务端为本次证明窗口分配该事务内严格递增的 `proofAttempt`，启动 proof deadline 后才启用 source。每名 still-current target 从实际 inbound RTP stats 与 decoded-frame counter 生成专用 codec-transition proof；proof 精确绑定 room/share、Viewer session、assignment/connection/media-binding/publication generation、codec-negotiation generation 与当前 `proofAttempt`，request 和 response 必须回显同一 attempt。Pause 清空当前 proof batch并使该 attempt 失效；下一次 Resume 分配新 attempt，因此任何较早窗口延迟到达的 proof 都不能提交当前窗口。不复用 Viewer quality evidence，也不改变 route authority。leave、hard failure 或 binding generation 变化会移除该 target并把仍在线 Viewer置为 `route-wait`；事务以剩余 target settle 后释放 owner，controller 再使用最终 committed preference 恢复该 Viewer。服务端只在全部 still-current target 的 actual codec 与 decoded progress 成立后提交 desired codec。Preparation failure 保持暂停并运行有界 old-preference rollback preparation；forward proof failure 先重新进入权威 paused，再运行 rollback preparation。Rollback prepared state 保留同一 owner且无 timer，等待用户下一次正常 Resume 后重验、分配新 proof attempt、启动 rollback proof deadline并启用 source；proof 证明 P2P 空/有序 preference 或 SFU fresh publish options 已恢复，并从新协商代次取得 actual inbound codec 与 decoded progress，不要求 actual codec 与旧观测字面相同。回滚失败保持暂停和明确错误，任何路径都不自动 Resume。stop/share replacement/room delete 清除 prepared state；零 target 只更新 future-edge desired codec。该 slice 是 paused bounded-gap mutation，不声称 make-before-break；`replaceTrack()` 或暂停本身不改变 codec，未暂停热切换后续复用同一事务。实际 outbound codec/profile、encoder implementation 与 stats 是结果真相；浏览器支持 H.264 不证明命中硬编。AV1 仍不得仅凭压缩率推断默认启用。唯一获准的 Web SDP 变换是下述 Viewer answer 的 Opus stereo receive preference，不得借此扩展视频码率、codec 或通用 munging。
