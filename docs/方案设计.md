...
code-only policy 使用单一 `open | password` 状态：`open` 是无房间密码的默认值；`password` 要求已通过 site access 的 Viewer 再输入该房间密码。两种状态都不改变有效 grant 的 exact-room Viewer authority，也不建立目录、账号、ACL 或额外媒体容量。Host 可独立改变 code-only policy、设置/替换/移除密码，以及 rotate/revoke grant。
