...
| Host HTTPS -> server | room-access update | 以 site access 与精确 Host token 独立设置 `open/password`、设置/移除 password，或 rotate/revoke grant；不建立媒体会话或续租，raw grant 只在新建或 rotate 成功响应中出现一次，服务端不回显密码 |
