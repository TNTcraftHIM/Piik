...
状态：Current source 使用单一 `screener-v10` Browser wire，默认 VP8，codec selector 仅在分享前用于诊断且结果在 share lifetime 内固定；Pause/Resume 由当前 Host session 和 `shareGeneration` 约束。Production 仍为 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`。精确 deployed behavior 只由 deployment 文档记录。
...
Web 视频端当前默认 `VP8`。诊断期内，`automatic` 以空 `setCodecPreferences([])` 使用 P2P 浏览器和 LiveKit 默认，显式 H.264/VP8 在本轮分享的首个 offer/publication 前应用；P2P 与 browser relay 保留浏览器公布的完整 codec/repair 列表，SFU 使用 `backupCodec: false` 的单一 publication codec。每次分享的 codec preference 保持固定，普通 Pause/Resume 只以当前 Host session 和 `shareGeneration` 更新 paused 状态并切换同一批 capture tracks。实际 outbound codec/profile、encoder implementation 与 stats 是结果真相。物理 direct 对照选择一个固定 Browser codec，browser relay 与 SFU 随后验证该选择，最终产品静态配置这一 codec。Viewer answer 的 Opus stereo receive preference 是当前 Web SDP 变换边界。
...
H.264 对照固定同一 Host、Chrome/Edge 版本、驱动、游戏场景、`1080p30 / 5 Mbps / balanced` 和单个有线 direct Viewer，并为 H.264、VP8 与复核用 H.264 分别创建全新 Host/share/PeerConnection 会话。质量判断只使用同一 generation 且时间重叠的 capture A、sender B、Viewer C 窗口，并同步记录游戏 FPS 与 Host 浏览器进程 CPU/GPU；stats identity poll 不推进采样 accumulator，可选 encoder 字段缺失保持 unknown。Capture、sender 与 Viewer 的实际 FPS/码率、codec/profile、encode/decode cost、`qualityLimitationReason`、丢帧、冻结和传输指标共同归类采集、编码、带宽或解码瓶颈。
...
- current source contract 使用单一 `screener-v10` 信令 literal、严格 JSON schema、方向与房间授权校验、payload 上限和 client-mismatch 刷新终态；分享状态由 `shareGeneration`-fenced paused boolean 表达，访问失败使用 `ROOM_ACCESS_DENIED`，Viewer route failure 为 typed terminal，Host 诊断按需返回脱敏 snapshot。`qualityProfileIdSchema` 保持三档，`qualityResolutionSchema` 包含 `480p`，`DEFAULT_QUALITY_SETTINGS` 为 1080p30、VP8；
...
- Current source 使用单一 `screener-v10` wire，默认 VP8，codec selector 只在分享前可用且对本次分享固定，Pause/Resume 由当前 Host session 和 `shareGeneration` 约束。Production 仍运行 exact `39fcf93bae057fcbb1002702c3be6b90bac9027f`、release `39fcf93`、wire `screener-v9`，回滚边界为 `/opt/screener/backups/39fcf93-pre-v9-20260824T004257Z`。v10 以 server 与 Browser assets 原子部署并在 room authority 前校验 wire version。
- 物理 direct 证据选定最终固定 Browser codec；若选择 H.264，再由 browser relay 与 SFU 验证。Host 后台性能按受控根因证据诊断；标准 ICE/SFU 按真实异构网络验收。
