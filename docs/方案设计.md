...
状态：production 运行精确 deployed application/runtime revision `679fe3e7af634309322bea83b316641f51ad3d09`、release `679fe3e`；current source 在同一严格 `screener-v11` runtime 上增加 Browser SFU ICE-server-isolation implementation `ae09c760adec76fd26da611d4928486d105c6d3b`，尚未部署。Browser display video 不设置 `contentHint`、audio 保留 `music`，唯一视频媒体 codec 为 VP8，UI、quality state/wire 和分享生命周期均没有 codec 分支。Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。
...
- 当前 production 要求 STUN，普通 peer ICE 只发 STUN servers；current source Browser SFU PC 使用空 ICE-server 列表、保留 LiveKit signaling 的 UDP candidates 并由标准 ICE 选择，该改动尚未部署；
...
- Production 运行精确 deployed application/runtime revision `679fe3e7af634309322bea83b316641f51ad3d09`、release `679fe3e`；current source 增加 Browser SFU ICE-server-isolation implementation `ae09c760adec76fd26da611d4928486d105c6d3b`，尚未部署。两者使用严格单一 `screener-v11` wire；Browser 固定 VP8 且 video 不设置 `contentHint`，codec selector、quality-state/wire 字段与分享生命周期 codec 分支不存在。
