...
SFU admission 在应用单进程中只有一个注入实例。LiveKit credential tuple 完整时，`SFU_INGRESS_CAPACITY` 与 `SFU_EGRESS_CAPACITY` 都必须显式配置为正 safe integer；它们没有产品默认值，也不从 Viewer 数、endpoint cap 或历史 root 数推导。账本只保存全局已用计数和按 exact room/share/publication generation 索引的 `reserved | committed` 条目：Host publication 计 ingress `1`，每个订阅计 egress `1`，候选与旧 generation 并存时按实际副本同时计数。route controller 先 reserve，再 prepare 和签发 token；media proof 后 commit candidate 并释放同房旧 generation，任何失败、超时、失联、分享换代、停止或删房都调用同一幂等 release。所有操作是 Map lookup 与整数加减，不扫描房间或参与者。当前切片不实现 TURN allocation admission，不改 parent selection，也不预建多进程协调；多应用进程部署必须先增加共享原子 owner。
