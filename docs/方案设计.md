...
5. 从 canonical `main` 按 ADR-0005 依次落 route waves A-D：先统一 endpoint cap/accounting 与 wire，再建立 SFU/TURN resource admission，最后把 deterministic allocation、child reparent 和 parent drain 收到一个 controller；每个 wave 独立可回滚。
