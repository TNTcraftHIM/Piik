...
最终分发有两个独立产物。公网服务器一键部署包面向用户自有服务器，编排同一版本的 Web/信令应用、易失房间状态、STUN/SFU/TURN 和反向代理，不建立第二套应用；域名、TLS、secret、端口、容量、健康检查和回滚继续 fail closed。它不提供托管基础设施，也不把占位配置写成可用服务。
...
WebSocket upgrade 无法预知后续 role，因此仍允许没有 site-access cookie 的合法 grant Viewer 完成握手；握手前严格校验 `/signal` 无 query、Origin allowlist、总连接数和未鉴权连接数，并把当次 cookie 校验结果保存到 socket state。五秒内的第一条 `authenticate` 完成授权：Host 必须同时拥有 upgrade 时有效的 site-access 状态和当前房间 `hostToken`；Viewer 必须拥有目标房间合法未过期 grant，或者先通过 site access 再按 `open | password | disabled` code-only policy 校验。site-access cookie 只允许进行 code-only 尝试，不能替代 grant/房间密码；Viewer grant 也不能用于 HTTP 建房、Host role、其他房间或其他操作。

Viewer grant 与 code-only 入口是两个正交维度。每个房间都用 `randomBytes(32)` 生成含 256-bit 随机量和到期时间的 room-scoped opaque grant，服务端只在内存保存完整 grant 的 SHA-256 摘要并用定长比较验证。Host 的主分享动作始终复制 token-bearing invitation；到期在以后鉴权/重连时拒绝，即时清退使用 rotate/revoke。

code-only policy 使用单一 `open | password | disabled` 状态：`open` 是无房间密码的默认值；`password` 要求已通过 site access 的 Viewer 再输入该房间密码；`disabled` 中性拒绝 code-only 尝试。它们都不改变有效 grant 的 exact-room Viewer authority，也不建立目录、账号、ACL 或额外媒体容量。Host 可独立改变 code-only policy、设置/替换/移除密码，以及 rotate/revoke grant。

Host 的同源 `localStorage` creation profile 保存显示名、code-only policy 和可选房间密码，使进程重启或租约过期后的下一次显式分享可原子应用同一偏好。该密码只保存在 Host 浏览器；不保存在 Viewer 浏览器、URL/query/fragment、cookie、日志、错误或服务器持久存储。建房/更新请求通过 HTTPS/WSS 把明文短暂交给服务端，服务端使用 Node 标准异步 `scrypt` 固定 `N=16384,r=8,p=1`、16-byte 随机 salt 和 32-byte verifier，只在当前房间内存保存 48-byte material。进程级 gate 最多执行 2 个、等待 16 个派生，队满即有界失败；任务开始和提交前都复核当前 Host/Viewer session 与 room material。不存在房间、未配置/错误密码和正确密码后的满房都使用通用拒绝。
...
- 一名在线分享者、最多 `20` 名 authenticated Viewer；房间 admission 与 endpoint/SFU/TURN capacity 分离。Production `9461e20` 默认八名且只接受 1 至 16；当前 source 已接受 1 至 20，生产值在 20-Viewer smoke 通过后随 route release 提升；
...
- 单个服务器内存控制面保存当前 room/participant/authorization/signaling 状态，但不记录 SDP、ICE candidate、IP 地址、媒体或 TURN 密钥；媒体面继续按 P2P-first 图分布，不把媒体压力移到应用服务器。
...
| 方向 | 消息 | 作用 |
| --- | --- | --- |
| client -> server | `authenticate` | Host 以 upgrade 时的 admission 状态加 room-scoped Host token 建立会话并可订阅 Web-only presence/settings；Viewer 以 exact-room Viewer grant，或 site access 加当前 `open/password` code-only policy 建立仅观看会话，并可附带规范化本地显示名 |
...
| host -> server -> host | room-access update | 独立设置 `open/password/disabled`、设置/移除 password，或 rotate/revoke grant；raw grant 只在新建或 rotate 成功响应中出现一次，服务端不回显密码 |
...
当前唯一 source wire 是 `screener-v8`，不做版本协商、旧 parser、双写或 translator。任何旧页面或 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。
...
TURN 只替换授权 logical edge 或 Host-SFU ingress 的 transport，不是拓扑节点。
现有 edge 优先保持 direct/STUN，必要时使用 exact selected TURN，再按需要切换到
SFU；Host 无 direct first-level path 时优先一条可经 TURN 的 Host-SFU
publication。端点容量、SFU/TURN ingress/egress、临时 overlap、server admission
...
与 quota 限界，并且不进入 URL、日志、browser storage 或任何持久存储。Coturn
...
- 访问入口：未鉴权根路径、`/join` 和 code-only `/r/{code}` 显示“站点访问”与“站点口令”，未知路径只显示“无法访问”。有效 `/r/{code}#v={grant}` 直接进入；其余 Viewer 通过 site access 后按 `open/password/disabled` 直接进入、看到“加入房间 / 房间密码”或得到中性拒绝。已有有效 site-access cookie 不重复显示整站验证；站点口令和房间密码不共享输入值或授权状态。无效、过期、错误密码或满员只显示通用拒绝，不泄露房间存在性或 policy。
- 分享者根页面：顶部显示“{分享者显示名} 的屏幕”和房间状态，并在标题旁显示可单独复制的紧凑四位房间号；Viewer 同一位置显示权威 Host 名称，不再在标题下重复。主区域保持稳定的 16:9 舞台。未发布、已停止或可恢复错误状态时，舞台内以同等尺寸和权重显示“开始分享”和“加入房间”，中间只用“或”与分隔线表达发送/接收；前者直接开始现有分享流程，后者作为 disclosure 在当前舞台内、紧邻触发按钮就地展开共享的房间码表单，提交后才导航到 `/r/{code}`。按钮同步 `aria-expanded`/`aria-controls`，展开后输入获得焦点，DOM 与焦点顺序保持触发器后接表单；窄屏允许表单换到动作行下方，不产生横向滚动。不增加首页、向导或拓扑选择。直播时舞台显示本地预览及独立全屏图标，底部保留停止、同步暂停音视频、质量档位，以及职责分离的“复制邀请链接”“复制房间号”“房间号加入：开放/密码/关闭”“更新邀请”“撤销邀请”控制；熟悉动作使用 lucide 图标、直接标签和 tooltip，不用一个含糊的“分享”菜单混装。主分享动作始终复制含 grant 的邀请链接。页面失焦仅暂停本地 video element，并在舞台提示“本地预览已暂停，分享仍在继续”；媒体暂停则禁用 capture stream 轨道，两者都保留现有连接。
- `/join`：保留为可直接访问的回退入口，并复用同一个四位房间码表单；通过 site access 后输入 `1000..9999` 并进入 `/r/{code}`，后续行为由房间 code-only policy 决定。
- `/r/{code}` 观看者：首次解析合法 fragment grant 后立即清理地址栏并直达；仅有 code 时先通过 site access，再按 `open/password/disabled` policy 进入。鉴权后主区域最大化远端视频，标题旁显示可复制的紧凑房间号，顶部保留连接状态，并提供本地音量、静音、全屏和重试控制；Host 主动暂停时复用视频舞台遮罩显示“分享者已暂停”，恢复后立即移除。视频舞台用单一派生 presentation reducer 合成 access、Host presence、route prepare/active、peer/SFU connection、remote track、首个 composited frame、autoplay rejection 和 bounded failure。支持时用 `requestVideoFrameCallback()` 确认第一帧已提交，旧浏览器回退到当前代 decoded-frame delta 与 video readiness；`playing`、track object 或 ICE `connected` 单独都不足以结束占位层。连接阶段占用固定布局且不显示虚假百分比，播放按钮只处理实际 autoplay rejection，失败重试只调用现有有界 controller 入口。
...
- RoomStore 的四位空闲码随机分配/释放、活动分享免过期、dormant lease/续租、Host token/Viewer grant 摘要验证、`open/password/disabled` 正交 policy、grant 到期/跨房拒绝、rotate/revoke 强撤销、Viewer 上限和超额拒绝；
...
- 单一 `screener-v8` 信令 literal、任意旧客户端 mismatch 的一次性刷新终态，以及协议非法 JSON、未知消息、错误方向、跨房间目标和 payload 限制；
...
- HTTP/WS 集成：nginx per-source site-access `limit_req` 与统一错误不泄露 secret，应用不建立 IP limiter 状态；Viewer 可在 Origin/容量门下 upgrade；Host 首条鉴权同时要求 upgrade-captured site access 与 Host token，Viewer 要么持 exact-room grant，要么先有 site access 再按 `open/password/disabled`。覆盖原子创建 profile、同浏览器重建偏好、fragment/sessionStorage/replaceState、无 site access 的 code-only 统一拒绝、通过 site access 后的 open/password/disabled 行为、raw grant/密码不进入 URL/日志/服务端持久状态，以及 rotate/revoke 后 grace/route 清理、旧 Viewer/上游媒体边关闭和 reconnect 拒绝；
...
- 一名分享者、20 名观看者先运行 bounded smoke，记录路由深度、首帧、sender 数、应用 CPU/内存和 SFU/TURN 用量；端点 `C=1/2/3` 分开解释。30 分钟异构公网 endurance 仍是后续质量证据，不阻止已经有界失败的 20-Viewer admission。
...
5. 在已合入的 endpoint cap/accounting、SFU lifecycle 与 TURN allocation admission 边界上，以单一 `screener-v7` wire 完成 route controller convergence：一个 event-driven reconcile 和一个 room-serial candidate 统一处理 deterministic allocation、child reparent 和 relay drain；该 wave 独立可回滚。
...
7. 加入 first-frame-driven Viewer connection stages；随后把房间收敛为四位随机码、易失租约、正交 code/grant access 和本地 Host creation profile，并原子删除 SQLite 双模式。
8. 当前发布只验收 Web Host、Viewer 和 browser relay。Native sender、capture helper 与 shared-encode executable 排在整体 roadmap 最后，不参与当前构建、路由验收或发布。
9. 当前功能、NAT 与真实网络 TODO 完成后，再分别设计和交付公网服务器一键部署包与完全不依赖外部服务的本地 Host-server 包。

生产已运行 `screener-v8` room/route release `352c457`。内部不兼容 wire/schema 原子切换，Git commit 与部署备份承担回滚，不保留兼容实现。
...
`reconcileRoom` 在没有 pending operation 时先删除 disconnected childless participant，再找一个 waiting Viewer，或 parent offline/effective-capacity 不足、exact edge 已失效的 child。Candidate planning 拒绝 stale session、不可 source-reachable、成环或没有稳态/overlap/bounded-gap producer slot 的 tuple；server resource admission 在执行当前 cursor 时单独检查。由 failed edge 启动时只为当前 fact version 把它的 `parent + transport` tuple记为已尝试；新的外部事实允许重新评估。对剩余 upstream 依次比较结果深度、剩余 sender slots、稳定加入顺序与 peer identity，先建立其他 direct/STUN，再建立 selected TURN 与 SFU 的有界顺序；同一 parent 的不同 transport 仍是不同 tuple。每次只 reserve/prepare cursor 指向的 candidate。WebRTC 负责 ICE/DTLS/consent；exact candidate child 解码出第一帧新画面后发送一次 ready，controller 先同步 commit typed admission，再原子提交 graph。失败释放 candidate、广播严格更新的 rollback revision 并推进 cursor，operation 总 deadline 不重置。耗尽时 hard-invalid、confirmed-departed 或 deterministic overflow edge 被退休并进入明确 waiting；healthy bootstrap edge 保留。无环和房间 admission 已约束最大深度，不增加独立 depth hard cap。

一个 operation 仍只有一个 timer 和一个总 deadline，但 timer 的下一次 wake 从当前 candidate list 派生。Controller 提取实际存在的 direct、selected-TURN、SFU 阶段并等分总窗口；显式失败可以在阶段内继续尝试其他 tuple，silent candidate 到达该阶段 wake boundary 后释放 reservation、广播 rollback 并跳到下一类。只有 direct 时仍使用完整 deadline；direct+SFU 时各用一半；三类同时存在时各用三分之一。这样不引入每-candidate magic milliseconds，也不允许一条 silent direct 把 SFU suffix 饿死。
...
所有非服务器端点共享一个 server-authoritative steady outbound media-copy cap `C`：唯一配置 `ENDPOINT_MEDIA_COPY_CAPACITY` 默认 `2`，只接受 `1`、`2` 或 `3`，不兼容旧键。服务端将 endpoint 当前广告的 `0..C` 可用值 clamp 为唯一 `effectiveDownstreamCapacity`；该值只表示当前可用 sender slots，不建立角色、浏览器、UA、设备或页面可见性 tier。peer child 或 Host publication 各占一份，upstream receive 免费，committed selected TURN 只替换同一份 edge transport；SFU subscriber egress 与 TURN allocation 使用独立 server admission。Browser `screener-v8` 的 `authenticated.endpointMediaCopyCapacity` 把部署 `C` 交给 Host 与 Viewer；两端用该值校验稳态 assignment，并让 active+pending 的物理 copy 只在 controller 已授权 overlap 时达到 `min(C + 1, 3)`。代码通过一个 shared accounting guard 执行，不重复 magic number；旧 wire 与 Native executable 在 authority 前拒绝，不保留兼容路径。Selected TURN 使用显式无默认值的正 safe-integer deployment capacity；同一个进程内 ledger 对每条 exact peer-selected edge 和 Host-SFU ingress 在 credential 签发前 reserve `1`，并让 reserved、committed、draining authorization 持续计费至 exact logical release。`C=3` 满载时先收到 active bounded-gap retirement再收到 prepare；任何会产生第四份 endpoint media copy 的 token/grant都不得签发。同一 edge 改用 TURN 不增加稳态 endpoint copy，但仍需独立 TURN admission。
...
只有实际因 deployment-wide SFU/TURN admission 失败的房间进入一个 `resourceWaiterRoomIds` Set。TURN logical release 或 LiveKit delete-plus-absent 使真实 usage 下降时，owner 一次 drain 该 Set，对每个仍存在的 controller 调用 `touchExternalFacts()` 并排入普通 reconcile。它不轮询、不按资源类型复制 route state，也不唤醒未等待资源的房间。
...
迁移使用 authenticated room/endpoint sessions、share generation、base/pending route revision 与 candidate connection identity。`route-update/prepare` 显式携带当前 operation 的 exact child peer ID、selected transport 与服务端生成的 candidate connection ID；parent 与 child 直接使用该 candidate tuple，即使同一 parent 只替换 transport、child 列表不变，也不得从 assignment 差异猜测本次 candidate。服务端在每个 WebSocket 上先发 prepare，再发其 selected-TURN grant 或 SFU config，并在允许 parent offer前先排入 child 的 companion；客户端依赖 WebSocket顺序而不维护消息重排 inbox。可用旧 route 保持 active，candidate 在 active map 外准备；candidate child 的第一帧新 decoded video 触发 `settleReady(commitAdmissionSync)`，成功 candidate `P` 下发 active `P`。失败、超时、代次变化或授权撤销销毁 candidate，保持仍有效的 committed graph 内容并先下发单调递增的 active rollback revision `R > P`，再准备后续 candidate；一个 monotonic allocator同时拥有 prepare/commit/rollback/retirement/prune revision。Direct edge 的 stock WebRTC rebuild可在 exact current sessions内原子采用新 connection ID而不改变 topology；selected transport随session变化走统一 retirement。Make-before-break 的 overlap 由统一端点容量、单个有界 `TransitionOverlapSlot` 和 typed server-resource admission 同时决定；stop/delete通过controller `dispose()`一次返回所有current与committed资源。

SFU/TURN 的 publisher、subscriber、selected transport、central admission、resource accounting、temporary overlap 和 fallback sequence 由 ADR-0005 统一定义。普通端点 child count 不能成为 server cap，兜底也不能形成无界 whole-room fanout。一个 Host publication 服务所有 SFU subscriptions；SFU-fed 与 peer-fed endpoint 使用相同的 provisional-child first-frame transaction。TURN 只替换授权 edge 的 transport，Host 无 direct first-level path 时可作为单条 Host-SFU ingress 的 transport。
...
实现迁移以当前 ADR-0005 invariants 为准；生产 release `9461e20` 仅是部署快照，代码迁移只保留仍满足当前 identity、first-frame commit 和 bounded-recovery 约束的部分。
## 官方与参考资料
