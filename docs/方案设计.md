...
质量自适应采用 ADR-0007 的路径隔离框架能力，而不是全房间按最差链路降档。Direct/peer 保留每条 PeerConnection 的 stock WebRTC GCC。Browser Host 的唯一 SFU publication 以固定 `HIGH+LOW` VP8 simulcast 提供两种空间表示，LiveKit Dynacast 负责停用未消费层，subscriber 的 stream allocator/BWE 独立选择实际下行层；`HIGH` 只是上限。Screener 不实现质量评分、带宽估计、周期调档或应用媒体层 selector。AdaptiveStream 只适用于 remote track 直接 attach 到实际播放元素且不承担 peer children 的 leaf Viewer；relay ingress 不能跟随本地 DOM 可见性。当前单层 `HIGH` 源码与生产保持不动，直到 pinned LiveKit 物理门禁证明层启停、弱下行选择和恢复行为符合该模型。
...
标准表示能力排在任何自定义媒体实现之前。Web P2P simulcast 缺少 receiver 选 RID 与跨 PeerConnection shared-encode 契约；LiveKit SFU 则原生拥有 per-subscriber 层选择与 Dynacast。Browser SFU 只在 ADR-0007 的 pinned-stack 门禁通过后发布固定 `HIGH+LOW`，并完全委托框架启停和选择；不得把该能力扩散到 direct/peer，也不得绕过 stock WebRTC congestion control。
