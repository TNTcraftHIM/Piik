...
WebSocket 使用无 query/fragment 的同源 `/signal`。upgrade 只记录 site-access cookie 是否有效；连接后五秒内第一条消息必须是 `screener-v2` 的房间级 `authenticate`。Host 消息携带 room、role、内部 Host token 与 clientId，并由 socket state 复核 site access；Web Host 可选发 `viewerPasswordSettings: true` capability，Native v2 不发送且不接收任何密码设置新消息。Viewer 消息携带 room、role、clientId，以及 private room 的 Viewer grant 或当前尝试的房间密码。服务端设置 64 KiB `maxPayload`、关闭 `perMessageDeflate`，并以 ping/pong 清理失效连接。单进程固定限制 2048 条总连接和 256 条未鉴权连接，容量在 upgrade 前预留，超限直接返回 HTTP 503 并销毁 socket。
...
Android 14/API 34+ 作为不阻断 Windows 主线的并行 P1 首片，只做一条窄纵向链路：
系统 `MediaProjection` picker -> `mediaProjection` foreground service -> libwebrtc
`ScreenCapturerAndroid` -> 仅暴露 VP8/H.264 的 `HardwareVideoEncoderFactory` -> 当前
site access/room/WSS/peer-assisted Host direct-child wire -> 一至两名未修改 Web
Viewer。不能使用含 software fallback 的 `DefaultVideoEncoderFactory`；找不到或创建
不了硬件 encoder 时明确失败。视频固定 720p30、3 Mbps `HIGH`，不增加移动端专属
拓扑、`LOW`、AV1、codec ladder、第三条 Host 下游边或跨平台 framework。

Android Host 以 authenticated snapshot 的 `routeRevision`/`routeAssignment` 为权威，
只为 `directChildPeerIds` 建立至多两条含 `targetPeerId` 的 STUN-only PeerConnection，
并用 assignment 给定的 connection generation 校验 answer/candidate/restart。较新的
direct `route-update(active)` 先 reconcile child，再发送 `route-ready(active)`；旧 revision、
未知 target 和 connection mismatch 均丢弃。首片不接 LiveKit SDK：snapshot 或 prepare
要求非空 `sfuPublicationGeneration` 时只发送一次相同 revision/phase 的 `route-failed`
并保持有界失败，不能谎称 production route 已发布。非 peer-assisted signaling 明确失败，
不能把本地旁路当作旗舰验收结论。

Android 的逐 session 授权、系统指示、旋转/尺寸回调、锁屏/撤销终止与资源释放
都是产品行为，不能隐藏或用重启 capture 掩盖 transport 问题。播放音频是默认关闭的
source extension：用户先从本应用可见的 launcher targets 中单独选择一个 UID；共享
UID 对应多个 packages 时不提供该目标。只有启用时才请求 `RECORD_AUDIO`，并用同一
`MediaProjection` 构造 `AudioPlaybackCaptureConfiguration`，同时匹配该 UID 与
`USAGE_UNKNOWN`/`MEDIA`/`GAME`。固定 `webrtc-sdk/android` 的公开
`JavaAudioDeviceModule.AudioBufferCallback` 把 48 kHz mono PCM16 注入一个 ADM；
`setAudioRecordEnabled(false)` 关闭 ADM 的平台麦克风输入，`AudioProcessingOptions.raw()`
避免语音处理。一个 `AudioSource`/`AudioTrack` 附加到当前至多两条 direct-child PC，
不修改 Viewer、服务端或路由 wire，也不为 SFU/selected-edge TURN 增加旁路。

Android 公开 API 34 的系统单应用 picker 不返回所选 package/UID，所以画面和音频是
两次独立选择；UI 必须直接说明两者可能不同，不能称为自动同应用。目标应用 opt-out、
protected content 或没有合格播放通常会让 `AudioRecord` 持续返回静音而非 read error；
track 存在只能显示“已请求选定 UID 的播放音频”，不能宣称实际有声。初始化/read
失败则关闭音频并保留视频，绝不回退到麦克风或全系统 mix。Android 官方播放捕获
契约要求 `RECORD_AUDIO` 和 `MediaProjection` token；服务只声明并启动
`mediaProjection` 类型，不申请描述为继续麦克风捕获的额外 `microphone` FGS 能力，
且 ADM 麦克风保持禁用。静音检测是 P2，真机 A/V sync、后台行为和设备差异仍是验收边界。
API 证据由[原生 sender 调研](./research/native-shared-encode-sender.md)和
[音频调研](./research/browser-screen-audio-quality.md)持有。
...
- 访问入口：未鉴权根路径、`/join` 和 code-only `/r/{code}` 只显示中性的站点访问验证，未知路径只显示“无法访问”。有效 `/r/{code}#v={grant}` 直接进入；其余 Viewer 通过 site access 后，public-watch 进入房间，private room 再显示中性房间密码输入。无效、过期、错误密码或满员只显示通用拒绝，不泄露房间存在性或 policy。
- 分享者根页面：顶部显示“{分享者显示名} 的屏幕”和房间状态，并在标题旁显示可单独复制的紧凑房间号；Viewer 同一位置显示权威 Host 名称，不再在标题下重复。主区域保持稳定的 16:9 舞台。未发布、已停止或可恢复错误状态时，舞台内以同等尺寸和权重显示“开始分享”和“加入房间”，中间只用“或”与分隔线表达发送/接收；前者直接开始现有分享流程，后者直接进入 `/join`，不增加首页、向导或拓扑选择。直播时舞台显示本地预览及独立全屏图标，底部保留停止、同步暂停音视频、质量档位和复制完整邀请链接；页面失焦仅暂停本地 video element，媒体暂停则禁用 capture stream 轨道，两者都保留现有连接。
- `/join`：通过 site access 后只输入正整数房间码并进入 `/r/{code}`；public-watch 直接通过，private room 缺失 grant 时显示中性房间密码表单。
- `/r/{code}` 观看者：private room 首次解析合法 fragment grant 后立即清理地址栏并直达；仅有 code 时先通过 site access，再按 public/private policy 进入。鉴权后主区域最大化远端视频，标题旁显示可复制的紧凑房间号，顶部保留连接状态，并提供本地音量、静音、全屏和重试控制。
- 移动端：视频保持稳定宽高比，控制与状态移到视频下方，任何文字和按钮不得溢出或遮挡。

两端后续默认显示紧凑在线 roster；Host 与 Viewer 的本机显示名默认只读，点击铅笔按钮才展开输入、保存和取消，展示仍为名称加 room-scoped `peerId` 短后缀。一个不持久化的“显示连接详情”复选框按页展开信令、ICE/TURN、浏览器实际暴露的本地/远端候选地址、codec、RTT、RTP 丢包率、编解码耗时及逐边统计；真实连接失败、音频缺失、持续画质受限和需要用户动作的告警始终可见。用户名/roster 不进入 access credential 或 SQLite。
...
- Android 14/API 34+ 原生 sender 首片代码闭环后只运行一次单设备/单 Web Viewer smoke：系统 picker、授权拒绝/撤销、一个 peer-assisted direct child、硬件 codec 选中、首帧与资源释放。旋转热更新、锁屏恢复、Wi-Fi/蜂窝切换、持续热降频、应用音频、secure/protected content 与多 Viewer 属于后续 gate；单应用窗口只在 Android 14 QPR2+ 测试；
...
8. P1 并行完成 Windows native fixed-`HIGH` staged gate 与 Android 14+ video-only sender 首片；二者共享现有控制协议但不预建跨平台 framework，也互不阻断。
9. Android 首片只接 peer-assisted direct children；SFU publication 明确有界失败。代码闭环后最多做一次单设备/单 Viewer smoke，性能、网络和设备矩阵后置。
10. iOS 27 ScreenCaptureKit final 后再决定是否运行独立 P2 spike；不实现 ReplayKit compatibility path。Viewer-to-TV 只采用已验证的平台本地输出，不改变媒体路由。
