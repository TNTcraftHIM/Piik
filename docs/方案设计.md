...
状态：首个 P2P PoC 已接受并实现；ADR-0003 在其上增加显式、实验性的单节点 SFU 模式，不代表任一模式已经通过性能验收。
...
本阶段不实现账号数据库、通用业务数据库、Redis、simulcast/SVC、观看者转发树、语音聊天、Electron、原生捕获、PWA、录制、遥测平台或多地区调度。唯一持久化例外是可选的 `node:sqlite` 房间身份表。后续最小扩展只加入由 `MEDIA_MODE=sfu` 显式启用的单节点 LiveKit，不加入自动切换、hybrid、双发、转码、E2EE 密钥系统或多节点设施。
...
| 可选 SFU | LiveKit Server、`livekit-client`、`livekit-server-sdk` | 使用成熟 SFU 和最小权限 join token；只在显式 SFU 模式加载浏览器 SDK |
...
不选 Go 重写应用服务的原因是默认 P2P 下 Node 不承载媒体，SFU 模式也由独立的 Go LiveKit 进程承担转发；重写控制面不会减少分享者或服务器带宽。不选 C++ 的原因是浏览器能力尚未被实测证明不足；只有捕获、应用音频或硬件编码出现明确瓶颈时才增加原生 helper。
...
可选 SFU 模式保持同一个 Screener 控制面，但媒体固定经过一个 LiveKit 节点：

```mermaid
flowchart LR
    H[分享者 Web] <-->|HTTPS/WSS: 房间与鉴权| A[Node 应用服务]
    V[观看者 Web 1..N] <-->|HTTPS/WSS: 房间与鉴权| A
    A -->|短期、房间/角色绑定 JWT| H
    A -->|短期、房间/角色绑定 JWT| V
    H -->|一份屏幕媒体| L[LiveKit SFU]
    L -->|每名观看者一份转发| V
```

`MEDIA_MODE` 在进程启动时固定为 `p2p` 或 `sfu`，省略时为 `p2p`。一个部署中的房间不会根据人数切换拓扑，也不会同时发布两条媒体路径。LiveKit room 是临时媒体对象；Screener RoomStore/SQLite 仍是房间身份、host 权限、人数和链接生命周期的唯一来源。

## 代码布局

```text
src/
  client/       React 页面、捕获、P2P peer、可选 SFU client、stats 和样式
  server/       HTTP/WS 入口、配置、房间、TURN 凭据和 LiveKit token
...
`ACCESS_PASSWORD` 是可选的全站开关。为空时网站公开；非空值必须包含 1 至 128 个可见 ASCII 字符（`0x21` 至 `0x7e`）。配置时，分享者访问根页面、观看者访问 `/join` 或 `/r/{code}` 都先进入同一个登录 gate。`POST /api/session` 校验密码后签发 12 小时的无状态 cookie：值只包含版本、到期时间和 HMAC-SHA256 签名，不创建账号、访问会话 JWT 或服务端 session Map，也不提供 logout 流程。cookie 使用 `HttpOnly`、`SameSite=Strict`、`Path=/` 与有限 `Max-Age`；HTTPS 生产环境再使用 `Secure` 和 `__Host-` 前缀。WebSocket 异常断开后会用 `/api/session` 检查一次当前 cookie；仅在确认失效时停止重连并返回 gate，网络故障仍走原重连流程。
...
| server -> client | `authenticated` | 返回角色、peerId、可为空的房间期限、容量、host 状态、连接快照和 `mediaMode`；只有 P2P 分支携带 ICE 配置 |
| server -> client | `sfu-config` | SFU 鉴权成功后异步返回公共 LiveKit URL 和短期、房间/角色绑定 token |
...
在 SFU 模式中，服务端拒绝 P2P 专用的 `signal`、`restart-request` 与 `refresh-ice`。`peer-joined`、`peer-left`、`host-status`、`stop-sharing` 和 `sharing-stopped` 仍由 Screener 控制面维护产品状态。观看端可在媒体初连失败或终态断开后发送 `refresh-sfu`，由当前已鉴权 socket 重新取得短期 token，不必重建房间。异步签发 token 后必须再次确认 socket/session 仍是当前代次，迟到结果不得发给已替换会话。LiveKit API secret 只存在于服务端配置；join token 不写入邀请链接或持久浏览器存储。LiveKit SDK 会在 `/rtc/` WebSocket 握手查询串中传递该 token，因此反向代理必须对该 location 禁用请求日志。
...
SFU 模式复用同一份 `getDisplayMedia()` 结果。host 使用固定 LiveKit identity `host` 加入从 Screener room ID 派生的媒体房间，只发布 `screen_share` 与 `screen_share_audio`，不能订阅或发送 data；viewer identity 包含当前 Screener peer ID，只能订阅。join token 有效期为五分钟，LiveKit room 首次创建时的 participant 上限随 Screener 的 viewer 上限加一。

分享者只发布一个视频 layer，首版显式关闭 simulcast，不强制 codec。切换来源通过 LiveKit local track 的 `replaceTrack()` 更新；音轨可按新来源增加、替换或移除。明确停止或捕获结束会断开当前 LiveKit publisher 并发出既有 `stop-sharing`，但不会删除 Screener 房间或邀请；观看者清空媒体并保持等待。普通 Screener WSS 短断不主动关闭仍健康的 LiveKit room，完整页面或媒体会话结束时才释放它。

viewer 只接收 identity 为 `host` 且 source 为屏幕视频或屏幕音频的 track，忽略其他参与者或 source。LiveKit room 断开、host 离开或 track 取消发布时进入等待，不把控制面短断误报为明确停播。

LiveKit 使用浏览器到 SFU 的 WebRTC 传输加密，但普通模式下 SFU 终止这些传输并可访问媒体，不属于排除服务器的 E2EE。LiveKit 的可选 E2EE 需要应用另行负责密钥生成、分发和轮换；全站密码不作为媒体密钥，本扩展不实现该系统。
...
6. 作为独立扩展加入进程级 `p2p|sfu` 选择、最小权限 LiveKit token 和两个薄客户端；先保持单节点、单 layer，再执行同条件 1/3/5/8 人对照。

任何一步只实现当前消费者需要的接口。不得为未来账号、第三种媒体拓扑或原生 sender 预建抽象层；P2P 与 SFU 只在页面入口按部署配置分支，不先建立通用媒体框架。
...
- [LiveKit JavaScript client API](https://docs.livekit.io/reference/client-sdk-js/)
- [LiveKit encryption and key distribution](https://docs.livekit.io/transport/encryption/)

MiroTalk BRO（AGPL-3.0）和 Screego（GPL-3.0）仅用于结构研究。Tailchat Meeting（Apache-2.0）用于捕获生命周期对照。LiveKit Server 与两个选用 SDK 均为 Apache-2.0；项目许可证确定前仍不复制 GPL/AGPL 代码。
