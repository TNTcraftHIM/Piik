...
`STUN_URLS`；生产缺失 STUN 启动即失败。`authenticated.iceConfig` 是静态
...
协商自己的 candidates。tracked LiveKit 配置只开放 UDP mux，不配置
`rtc.tcp_port`、external TURN 或 embedded TURN。Node 应用不能替代部署
验证来证明该 UDP listener 从公网可达。
...
保留 legacy TURN 分支。`PEER_ASSISTED_ROOM_IDS` 只灰度 topology/SFU route
...
- 自托管 coturn 必须启用 `stun-only` 并关闭 TCP、TLS 和 DTLS listeners；不得配置 relay port range、TURN auth secret 或匿名 TURN。未来 selected-edge TURN 由独立设计取代这份 STUN-only 配置，而不是在当前 wire 上加回全房 credential。
...
- CGNAT、双 NAT、移动热点和普通家宽下使用隔离 candidate，LiveKit 显式关闭 TCP fallback 且不配置 TURN，验证 host 与 roots 使用 SFU/UDP；人为封锁 UDP 时必须有界明确失败。旧 production release 留在独立实例作为回滚，不在同进程保留 legacy transport；
