...
状态：首个 PoC 基线及其已接受扩展的当前设计。Production `d6c8aa0` 已使用 STUN-only ordinary ICE，并只为 room `1` 启用 exact-room peer-assisted 与 LiveKit SFU/UDP；`769de201f7cc` 与 coturn relay 仅保留为隔离回滚路径，二者不是同进程兼容分支。
...
旧 release `769de201f7cc` 需要 coturn TURN/UDP+TCP，目前仅作为
独立回滚实例保留；current production 不根据 room allowlist
...
产品硬目标是任一非服务器节点任何时刻最多两条下游媒体边。系统按一个或两个观看者直接 P2P、更多观看者 peer-assisted、部署 SFU 中央兜底的顺序自动分配和恢复，分享者与观看者不选择拓扑。默认媒体 transport 只使用 UDP；SFU 作为 virtual parent 正常只送一至两个 roots，roots 之后仍优先沿受限 peer descendants 扩散；多个无法挂到健康 root 的极端 viewers 只可使用另设上限的中央 edge。Production `d6c8aa0` 已把普通 ICE 收敛为 STUN-only、删除全房 TURN wire并只为 room `1` 开启 failure-only controller；exact-room 公网媒体仍未验收，旧 release/coturn 仅作回滚。未来可选 TURN 只能按需给异常 assignment/connection generation，由单独完整 PR 实现。ADR-0004/0005 的 broad-rollout 验收尚未完成。
