...
状态：已接受 Browser display video 不设置 `contentHint`，audio 保留 `music`；最终 Browser codec 尚未决定。Current source 与 production 仍是 `screener-v10`、默认 VP8，并保留对每次分享固定的临时诊断 selector。Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。精确 deployed behavior 只由 deployment 文档记录。
...
下表描述 current Browser 的单一 `screener-v10` wire；server 与 Browser assets 必须作为一个完整合同原子发布，v9 和更早客户端在取得房间 authority 前拒绝，不存在旧 parser、writer、translator 或中间协议子集：
...
current 的唯一 wire 是 `screener-v10`，不做版本协商、v9 parser、双写或 translator。任何 v9 或更早页面及 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
浏览器可返回更低设置，界面以 `track.getSettings()` 显示实际捕获结果。推荐区始终只有表中的三档并默认选择中档 `1080p30`；高级分辨率额外提供 `480p`（854x480）、720p、1080p 与 1440p，FPS 继续独立接受 15 至 60 整数，码率继续独立接受 2 至 12 Mbps，因此不建立第四个推荐档或 `480p` preset ID。视频 track 不赋任何 `contentHint`，让 display capture 保留浏览器的 screen 分类；返回的音频 track 使用 `contentHint = "music"`。三档推荐质量与折叠高级区初始值统一采用 `degradationPreference = "balanced"`，并允许显式选择 clarity-first 或 fluid。当前 v10 的“分享高级设置”暂时保留仅分享前可用、对本次分享固定的诊断 codec selector，并提供离散的 `64 kbps 省流 | 128 kbps 音乐默认 | 256 kbps 极高音质` 屏幕音频 sender ceiling。音频 ceiling 可在直播中串行更新。这些设置只是 ceiling 和浏览器偏好，不保证实际分辨率、帧率、码率或声道。
...
当前 v10 Web 视频端默认使用 `VP8`。P2P、browser relay 与 SFU publication 使用分享开始时由临时诊断 selector 选定的同一 codec，且本轮分享内固定；不做运行时或自动 codec 切换。最终 codec 等待无 hint 的真实游戏负载证据后决定，再原子删除 selector、quality-state/wire 字段和 share-lifetime 分支。实际 outbound codec/profile、encoder implementation 与 stats 是只读结果真相。Viewer answer 的 Opus stereo receive preference 是当前唯一应用 SDP 变换边界。
...
codec 与 content-hint 依据由研究文档持有。`motion` 会把 Chromium/libwebrtc 的 display capture 从 screen 改成 realtime，并在同一 VP8 条件下造成明显降帧和降分辨率，因此产品不设置视频 hint。无 hint 时，普通 AMD H.264 MFT 的浏览器码控仍丢帧，而诊断性 Desktop SW BRC 可恢复约 30 fps，说明问题不等于硬件编码器本身的吞吐上限；网页没有启用该浏览器进程开关或选择具体 GPU/MFT 的 API。最终 codec 仍需真实游戏画面、整机负载和接收质量证据，实际 codec 与 encoder 字段只用于诊断，不触发 codec 切换。
...
- 已接受 Browser video 不设置 `contentHint`；最终 codec 未决，当前 v10 selector/wire 暂时保留。Host 后台性能按受控根因证据诊断；标准 ICE/SFU 按真实异构网络验收。
