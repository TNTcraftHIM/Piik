...
状态：已接受 Browser display video 不设置 `contentHint`、audio 保留 `music`，且 Browser 唯一视频媒体 codec 为 VP8。下一原子 Browser 合同是严格 `screener-v11`，不含 codec selector、quality-state/wire 字段或分享生命周期 codec 分支；current source 与 production 仍是 `screener-v10`，production 还设置视频 `motion`。Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。精确 source/deployed behavior 只由 status 与 deployment 文档记录。
...
- 接受的下一合同使用单一 `screener-v11` wire，Browser 固定 VP8 且 video 不设置 `contentHint`；codec selector、quality-state/wire 字段与分享生命周期 codec 分支不存在。Current source 与 production 仍是 `screener-v10`，production 还设置 video `motion`；实现与部署分别原子推进。
