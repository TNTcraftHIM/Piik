...
Accepted `screener-v12` Web 视频固定使用 `VP8`。P2P 与 browser relay 在首次 offer 前只保留 VP8 media capabilities 及 RTX/RED/FEC repair capabilities；无法应用该约束时明确失败，不静默协商另一媒体 codec。其唯一 active VP8 video section 使用结构化 SDP 把 `x-google-start-bitrate` 设为所选 screen-share target 的 90%，与 pinned LiveKit 的上游初始画质机制一致；非唯一、缺失或不可解析形态保留浏览器原 offer，实际发送仍由 sender ceiling 与原生 BWE 决定。SFU publication 显式使用 VP8 且关闭 backup codec。UI、quality state/wire 和分享生命周期均没有 codec 分支；实际 outbound codec/profile、encoder implementation 与 stats 是只读结果真相。应用 SDP 变换边界只有该 raw-peer VP8 startup hint 与 Viewer answer 的 Opus stereo receive preference。
