...
Web 端不做 SDP munging。P2P 与浏览器 relay 在首个 offer 前读取标准 sender capabilities；存在 H.264 时用 `setCodecPreferences()` 将全部 H.264 条目置前，同时保留完整的 browser codec/repair 列表，能力缺失或 API 拒绝时维持浏览器默认。SFU publication 同样在 H.264 能力存在时请求 `videoCodec: "h264"`，否则请求 VP8，并始终保持 `backupCodec: false`，因此不会为兼容回退同时维护第二份 codec publication。实际 outbound codec/profile 与 stats 是协商结果真相。这个可逆偏好只要求定向互通和回退验证；硬件、CPU/GPU、游戏 FPS 和目标网络证据用于性能结论及未来动态选择，不作为上线前置。AV1 仍不得仅凭压缩率推断默认启用。
