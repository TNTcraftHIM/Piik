...
- CGNAT、双 NAT、移动热点和普通家宽下使用隔离 candidate，LiveKit 不配置 `rtc.tcp_port` 或任何 TURN，验证 host 与 roots 使用 SFU/UDP；人为封锁 UDP 时必须有界明确失败。旧 production release 留在独立实例作为回滚，不在同进程保留 legacy transport；
