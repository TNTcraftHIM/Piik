...
5. 现有 automatic peer/SFU controller 作为 legacy deployed input；按 ADR-0005 迁移其 server-assisted route semantics，并让 room `1` 仅作为历史 smoke fixture。
6. 按 ADR-0005 的整体路由模型迁移 server-assisted transport；ordinary STUN-only、SFU 与 TURN 的配置和 rollout 保持独立边界。
