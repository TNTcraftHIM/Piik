...
状态：首个 PoC 基线及其已接受扩展的当前设计。Production 普通 peer ICE 保持 STUN-only，并已部署 ADR-0005 的完整 Browser route controller 与易失房间模型。下一 Browser v9 与 live-media mutation 已接受但尚未实现或部署；精确 deployed behavior 只由 deployment 文档记录。
...
最终分发有两个独立产物。公网服务器一键部署包面向用户自有服务器，编排同一版本的 Web/信令应用、易失房间状态、STUN/SFU 和反向代理，不建立第二套应用；域名、TLS、secret、端口、容量、健康检查和回滚继续 fail closed。它不提供托管基础设施，也不把占位配置写成可用服务。
...
下表先列 current v8 已实现消息，再以 `v9` 明示 accepted delta；实现分片可以排序，但 server 与 Browser assets 只发布一个完整 v9 wire，不存在中间协议：
...
| server -> viewer | `route-status` | v9 strict union 只允许 `{ revision, state: "waiting", reason: "sfu-admission" }` 或 `{ revision, state: "failed", reason: "route-exhausted" }`；只补充现有 `route-update` 无法表达的 SFU admission waiting 与 bounded route exhaustion，不携带 raw text、identity、candidate 或 retry hint |
| host -> server -> host | `request-route-diagnostic` / `route-diagnostic-snapshot` | v9 仅在 authenticated Host 点击下载诊断时请求并返回一份当前快照；不连续推送、不参与 route authority |
| host/viewer -> server | codec transition apply/proof | v9 精确绑定 share/codec generation 与 frozen media binding，独立于 `route-ready` 和 Viewer quality evidence |

Zod 在服务边界拒绝未知类型、超长字符串、非法 SDP/candidate 结构和越权目标。服务端根据已认证会话决定路由：观看者不能给另一名观看者发信令，分享者也不能向本房间以外的 peerId 发送。

当前唯一 source wire 是 `screener-v8`，不做版本协商、旧 parser、双写或 translator。任何旧页面或 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。Accepted v9 对已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和 close 行为。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
浏览器可返回更低设置，界面以 `track.getSettings()` 显示实际捕获结果。推荐区始终只有表中的三档并默认选择中档 `1080p30`；高级分辨率额外提供 `480p`（854x480）、720p、1080p 与 1440p，FPS 继续独立接受 15 至 60 整数，码率继续独立接受 2 至 12 Mbps，因此不建立第四个推荐档或 `480p` preset ID。视频 track 使用 `contentHint = "motion"`，返回的音频 track 使用 `contentHint = "music"`；二者都是内容提示而不是 codec 或质量档位。三档推荐质量与折叠高级区初始值统一采用 `degradationPreference = "balanced"`，并允许显式选择 clarity-first 或 fluid。“分享高级设置”同时提供 `自动 | H.264 | VP8` 和离散的 `64 kbps 省流 | 128 kbps 音乐默认 | 256 kbps 极高音质` 屏幕音频 sender ceiling。音频 ceiling 可在直播中串行更新；视频 codec 首先支持在分享显式暂停时事务化切换。它们只是 ceiling 和浏览器偏好，不保证实际分辨率、帧率、码率、声道或硬编。
...
Web 视频端不做视频 SDP munging。`automatic` 以空 `setCodecPreferences([])` 恢复 P2P 浏览器默认且不覆盖 LiveKit 默认 codec；显式 H.264/VP8 在首个 offer/publication 前应用，P2P 与 browser relay 仍保留完整 browser codec/repair 回退列表，SFU 始终保持 `backupCodec: false`。暂停时的 codec 切换使用单调 codec-negotiation generation，并和 route mutation 串行：冻结 exact current route/media binding target，新 join 按 pause 语义等待、leave 从 target 移除；P2P/relay 在暂停中以独立总 deadline 显式重新 offer/answer，SFU 以新 publication generation republish。SFU bounded-gap 先以 active revision 权威退休旧 generation，并等待该 managed room delete-plus-absence 完成，之后才创建 room、签发 token 或连接新 generation；全局 SFU 尚有余量也不能形成未获准的双 publication overlap。Preparation 成功后只停止 preparation deadline；同一个 room-serial codec mutation owner 保留一个内存有界的 prepared generation，暂停期间无 timer并排斥 route mutation，绝不擅自恢复用户主动暂停的分享。Host 的正常 Resume 动作先在该 owner 内原子重验 frozen bindings，由服务端为本次窗口分配进程内严格递增的 `resumeAttempt` 并启动同一个有界 resume/proof deadline；服务端向 Host 发恢复授权，但继续保持 authoritative pause。Host 成功启用 source 后回发精确绑定 room/share、Host session、codec generation（可空）与 `resumeAttempt` 的 source-enabled acknowledgement；只有服务端接受该 ack 后才清除 authoritative pause、先向 Viewer 广播 Resume，再下发 codec proof request。每名 still-current target 从 ack 后的实际 inbound RTP stats 与 decoded-frame counter 生成专用 codec-transition proof；proof 精确绑定 room/share、Viewer session、assignment/connection/media-binding/publication generation、codec-negotiation generation 与当前 `resumeAttempt`，request 和 response 必须回显同一 attempt。Pause 清空当前 proof batch并使该 attempt 失效；下一次 Resume 分配新 attempt，因此任何较早窗口延迟到达的 ack 或 proof 都不能提交当前窗口。不复用 Viewer quality evidence，也不改变 route authority。leave、hard failure 或 binding generation 变化会立即使 target 退出 proof 并把仍在线 Viewer置为 `route-wait`；在 codec owner settle 前，其旧物理 edge 与资源仍按实际占用保守计数，commit/abort 用同一个下一 active revision 统一 teardown 和释放，不能以未广播的逻辑失效低估 endpoint 或 SFU 用量。事务以剩余 target settle 后释放 owner，controller 再使用最终 committed preference 恢复该 Viewer。服务端只在全部 still-current target 的 actual codec 与 decoded progress 成立后提交 desired codec。Preparation failure 保持暂停并运行有界 old-preference rollback preparation；forward proof failure 先重新进入权威 paused，再运行 rollback preparation。Rollback prepared state 保留同一 owner且无 timer，等待用户下一次正常 Resume 后重验、分配新 resume attempt、完成 source-enabled ack、启动 rollback proof并启用 source；proof 证明 P2P 空/有序 preference 或 SFU fresh publish options 已恢复，并从新协商代次取得 actual inbound codec 与 decoded progress，不要求 actual codec 与旧观测字面相同。回滚失败保持暂停和明确错误，任何路径都不自动 Resume。stop/share replacement/room delete 清除 prepared state；零 target 只更新 future-edge desired codec。普通无 codec 的 Resume 也使用同一个 bounded `resumeAttempt` authorization/ack boundary，ack 前不向 Viewer宣告恢复，attempt timeout、Pause、stop、share replacement 或 Host session replacement 都保持 authoritative pause并允许用户重试。该 slice 是 paused bounded-gap mutation，不声称 make-before-break；`replaceTrack()` 或暂停本身不改变 codec，未暂停热切换后续复用同一事务。实际 outbound codec/profile、encoder implementation 与 stats 是结果真相；浏览器支持 H.264 不证明命中硬编。AV1 仍不得仅凭压缩率推断默认启用。唯一获准的 Web SDP 变换是下述 Viewer answer 的 Opus stereo receive preference，不得借此扩展视频码率、codec 或通用 munging。
...
v9 在 authenticated Host 点击下载诊断时发送一次 `request-route-diagnostic`；服务端当场返回严格 Host-only 的 `route-diagnostic-snapshot`，不连续推送。每个 current child 只保留 latest demand timestamp、operation/candidate start、first-frame/final outcome；新的 route demand 覆盖该 child 的旧 record，confirmed departure 与 room delete 立即删除，且不增加 timer、事件环或第二张 graph。`queueWaitMs` 只从同一个 current demand 到其 room-serial operation start计算；没有权威 demand timestamp 就是 `null`。Serializer 为本次导出分配 snapshot-local opaque ordinal，用它表达 current parent/SFU 关系、effective capacity、child count、current operation reason/stage/cursor/candidate count与相对 timing；`finalRoute` 只允许 `direct | sfu | waiting | failed`，`rejectionBucket` 只允许 `none | stale | endpoint-capacity | sfu-admission | candidate-failed | first-frame-timeout | operation-deadline | aborted`。Ordinal 不是身份或稳定 ID。Snapshot 不包含 room code、display name、真实 peer/session/connection/generation、地址、URL、SDP、token、raw error或 planner 曾过滤的 parent，不形成路由输入或前台第二套状态。Viewer 的 schema v2 只携带自身 stage、粗粒度 revision、typed failure、signal/Host state、route kind 与 current-frame proof，即使 connection list为空也可下载；未鉴权 `ROOM_ACCESS_DENIED` 分支只含 stage/code，不含 Host、route、revision或timing。
...
| 优先级 | 权威输入 | 展示与动作 |
| ---: | --- | --- |
| 1 | stale client、room closed/expired/lost、`ROOM_ACCESS_DENIED` | 阻塞终态；只显示对应刷新、可选密码或使用 Host 邀请的动作，不显示 route retry |
...
| 5 | route terminal failure、Host stopped/offline | 只有仍 current 且已证明的旧画面可保留；否则阻塞。仅 route/controller 明确允许时显示 bounded retry |
...
- 下一不兼容实现 checkpoint 的单一 `screener-v9` 信令 literal、任意旧客户端 mismatch 的一次性刷新终态，以及协议非法 JSON、未知消息、错误方向、跨房间目标和 payload 限制；v9 原子删除无消费者的 parent-edge quality proof，加入 paused codec transaction proof、`ROOM_ACCESS_DENIED`、typed Viewer route failure 与 Host-only 按需脱敏 route snapshot，不保留 v8 parser、writer 或别名；`qualityProfileIdSchema` 保持三档不变，只有 `qualityResolutionSchema` 增加 `480p`，`DEFAULT_QUALITY_SETTINGS` 改为 1080p30；
...
- HTTP/WS 集成：nginx per-source site-access `limit_req` 与统一错误不泄露 secret，应用不建立 IP limiter 状态；Viewer 可在 Origin/容量门下 upgrade；Host 首条鉴权同时要求 upgrade-captured site access 与 Host token，Viewer 要么持 exact-room grant，要么先有 site access 再按 `open/password/disabled`。覆盖原子创建 profile、同浏览器重建偏好、fragment/sessionStorage/replaceState、无 site access 的 code-only 统一拒绝、unknown/expired/disabled/wrong-or-absent-password/full code-only 使用完全相同的 `ROOM_ACCESS_DENIED` code/文案/close、grant 与 Host 的独立错误、raw grant/密码不进入 URL/日志/服务端持久状态，以及 rotate/revoke 后 grace/route 清理、旧 Viewer/上游媒体边关闭和 reconnect 拒绝；
...
- Viewer stage reducer：按上表覆盖 access/Host/queue、peer/SFU prepare、remote track 无首帧、首个 composited frame、autoplay rejection、健康媒体上的 signaling recovery、Host stop/offline 和 bounded failure；stale revision不能覆盖 current frame，raw server message不能进入 UI，零连接失败仍可导出 allowlisted stage/revision/code bucket；
- route parser 覆盖 `/r/`、三位/五位、`0000`、ASCII 字母、Unicode 数字、额外 path segment、合法 query/hash 与 exact `1000..9999`，且 malformed 输入不发起鉴权；Host-only diagnostic request 拒绝 Viewer，schema 拒绝额外/raw/private 字段，departure/room delete清除 latest timing，zero-connection Viewer export仍成立；
...
- CGNAT、双 NAT、移动热点和普通家宽下按 accepted route model 验证 direct/peer、SFU/UDP、恢复和 bounded failure；记录 RSS/CPU/latency 与 RX/TX/loss。人为封锁全部 UDP 时仍必须有界明确失败。旧 production release 留在独立实例作为回滚，不在同进程保留旧 transport；
...
4. 完成连接恢复和 stats 面板，并保持 ordinary peer 的 STUN-only snapshot。
5. 在 endpoint cap/accounting 与 SFU lifecycle 边界上，以单一 Browser wire 完成 route controller convergence：一个 event-driven reconcile 和一个 room-serial candidate 统一处理 deterministic allocation、child reparent 和 relay drain；该 wave 独立可回滚。
6. 已部署路由与易失房间边界保持不变；在同一个尚未部署的 Browser v9 checkpoint 中依次实现 live screen-audio ceiling 和 paused video-codec transaction。
7. 在该 checkpoint 中删除未消费的 parent-edge quality proof，加入 first-frame-driven Viewer stage/typed failure、按需脱敏 route timing、无枚举房间准入结果，并把默认推荐档改为 1080p30、只在高级分辨率增加 480p；全部 v9 protocol source 一起原子部署，不发布中间 wire 子集。
8. 对 H.264/模糊与 Host 后台性能只做受控根因诊断；标准 ICE/SFU 按真实异构网络验收，不增加 guessed candidate、NAT classifier、TCP probe 或 quality-driven reparent。
9. 当前发布只验收 Web Host、Viewer 和 browser relay。Native sender、capture helper 与 shared-encode executable 排在整体 roadmap 最后，不参与当前构建、路由验收或发布。
10. 当前功能与真实网络 TODO 完成后，再分别设计和交付公网服务器一键部署包与完全不依赖外部服务的本地 Host-server 包。

生产当前运行 `screener-v8` room/route release `8f5b3f1`；上述 v9 是 accepted design，尚未实现或部署。内部不兼容 wire/schema 原子切换，Git commit 与部署备份承担回滚，不保留兼容实现。
