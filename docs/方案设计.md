...
质量自适应采用 ADR-0007 的路径隔离框架能力，而不是全房间按最差链路降档。Direct/peer 保留每条 PeerConnection 的 stock WebRTC GCC。Browser Host 的唯一 SFU publication 以固定 `HIGH+LOW` VP8 simulcast 提供两种空间表示。LiveKit Dynacast 按全房最高需求停用其上的发布层，无订阅时停用全部层；启用 send-side BWE 的 server stream allocator 独立选择每个 subscriber 的实际下行层，`HIGH` 只是上限。Screener 不实现质量评分、带宽估计、周期调档或应用媒体层 selector。当前任一 Viewer 都可能承担 peer children，因此 subscriber 统一保持 `adaptiveStream: false`，relay ingress 不跟随本地 DOM 可见性。Pinned gate 已证明受限下游原生选择 LOW、非受限下游取得 HIGH、手动及元素驱动的同连接恢复和 Dynacast 启停；也证明任何 HIGH 需求会让 LOW+HIGH 同时编码。双编码是支持受限下游的固定成本，不是回退到单层 HIGH 的理由。当前源码与生产尚未迁移，仍是单层 `HIGH`。
...
标准表示能力排在任何自定义媒体实现之前。Web P2P simulcast 缺少 receiver 选 RID 与跨 PeerConnection shared-encode 契约；LiveKit SFU 则原生拥有 per-subscriber 层选择与 Dynacast。ADR-0007 的 pinned-stack 门禁已通过，Browser SFU 的接受目标是固定 `HIGH+LOW`、Dynacast 和 server send-side BWE，并完全委托框架启停和选择；不得把该能力扩散到 direct/peer，也不得绕过 stock WebRTC congestion control。
...
- Production 使用严格单一 `screener-v12` wire 和精确 deployed application/runtime revision `1d8761528d0dba43fb6d818df3934483ba2f5340`、release `1d87615`；current source 含相同 application/runtime tree 与当前 truth checkpoint。两者均已实现短 room-lived grant、明确 missing-room 结果、产品诊断下载移除、完整短 URL 邀请 UI、exact-current audio stats 与音轨跨 relay/SFU reconnect 连续性，固定 Browser VP8 且 video 不设置 `contentHint`，codec selector、quality-state/wire 字段与分享生命周期 codec 分支不存在；Browser SFU 只发布一个 `HIGH` encoding，不启用 simulcast 或 Dynacast。
