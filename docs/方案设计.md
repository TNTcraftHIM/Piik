...
状态：canonical source 已实现唯一 strict `screener-v13` 的 evidence-only quality shadow，production 在原子应用发布前仍使用 strict `screener-v12`；旧页面在 authority 前拒绝，不保留 v12 parser。当前已实现每分享代次的 H.264 sender gate 与 VP8 fallback、本地 pre-share `VP8 | 自动 | H264` selector、可选 SQLite stable room authority、无期限 preferred code 和换房控制；这些不新增 codec wire、自定义 SFU layer list或应用媒体质量 controller。Video 使用 `contentHint = "motion"`，audio 使用 `music`。SFU 由 pinned LiveKit 默认 representations、Dynacast、server send-side BWE 和 disabled AdaptiveStream 拥有质量适配；screen audio 使用 64/128/192 kbps ceiling、stereo、DTX off 与 RED off。精确执行状态由 `docs/status.md` 持有。
...
- 创建四位随机数字房间；production site access 密码严格控制建房/发布和 code-only 观看，默认 room-scoped capability link 让受邀 Viewer 直接观看；轻量模式把房间权威留在当前进程，可选 SQLite stable 模式只持久化房间权威 aggregate；
...
当前不实现账号数据库、通用业务数据库、Redis、语音聊天、Electron、原生捕获、PWA、录制、遥测平台或多地区调度。可选 SQLite 只保存 ADR-0002 定义的房间权威，不保存参与者、路由或媒体；peer-assisted/SFU candidate 只通过 ADR-0004/0005 的现有窄 owner 扩展，不引入第二套路由器或通用优化框架。
...
Host 的同源 `localStorage` creation profile 保存显示名、code-only policy 和可选房间密码，使 ownership 失效或租约过期后的下一次显式分享可原子应用同一偏好。建房响应仍返回实际 `roomLeaseSeconds` 并仅由 Host ownership/服务端房间生命周期使用；独立 preferred-room 记录只保存上次实际分配的四位号码，不保存时间、不运行续写 interval，也不证明或延长服务端租期。旧 ownership 过期/失效后始终请求该号码，服务端只在空闲时采用，否则随机分配并让客户端覆盖本地偏好。该密码只保存在 Host 浏览器；不进入 Viewer 浏览器、URL/query/fragment、cookie、日志或错误。建房/更新请求通过 HTTPS 把明文短暂交给服务端，服务端使用 Node 标准异步 `scrypt` 固定 `N=16384,r=8,p=1`、16-byte 随机 salt 和 32-byte verifier；稳定模式只持久化这 48-byte verifier material，轻量模式只保存在当前 RoomStore。进程级 gate 最多执行 2 个、等待 16 个派生，队满即有界失败；任务开始和提交前都复核当前 Host authority 与 room material。不存在、已回收或已过期房间使用 `ROOM_NOT_FOUND`；现存房间的未配置/错误密码和正确密码后的满房使用 `ROOM_ACCESS_DENIED`。
...
房间只有一个权威模型和两种存储模式。进程启动时以 `1000..9999` 建立空闲码池，并先扣除稳定模式恢复的未过期房间；创建房间可携带一个严格四位 `preferredRoomId`，若它仍在空闲池则以同一个 swap-remove 取走，否则用加密随机索引分配，释放时放回。偏好请求不预占、不替换现存房间，也不改变新房凭据生成。固定 9000 个活动房间容量不需要碰撞重试、递增身份、独立房间上限或数据库序列。

`ROOM_LEASE_SECONDS` 默认 86400。RoomStore 对连接中的 Host 不执行到期；Host 明确停止或连接消失时将 `expiresAt` 设为 `now + lease`，同一 Host token 在期限内重新鉴权并分享时续租，Viewer 活动不修改期限。清理循环删除 dormant expired room、关闭其 session/route/resource 并把号码归还空闲池。`ROOM_DATABASE_PATH` 缺失时全部 authority 只在内存；配置绝对文件路径时，一个 exact-version `node:sqlite` owner 事务持久化 roomId、Host/grant digest、authorization generation、policy、可选 verifier 和 lease。启动删除过期行，保留 dormant absolute deadline，并把 crash-active 行转换为 `startup now + lease`；participant/session/share/pause/quality/codec/route/SFU/stats 一律不恢复。错误 application id、schema、row、路径或第二 owner 在 signaling/LiveKit 前 fail closed，不维护旧 schema reader 或 migration chain。所有房间继续生成 256-bit Host token，Host 浏览器保存 raw token，服务器只保存摘要；不使用 IP、UA、设备信息或浏览器指纹认领。

rotate/revoke 是 grant 路径的强撤销：服务端先在 RoomStore 中原子提交新 digest/authorization generation（稳定模式同一 SQLite transaction），再清除 Viewer grace、连接代次与路由状态，向 Viewer 发送通用访问已撤销终态使其立即关闭接收 `RTCPeerConnection`/媒体并停止重连，同时让每个 Host/relay/SFU upstream owner 关闭旧代次发送边，最后关闭 Viewer socket。不能只断 WSS，因为健康 P2P 可在信令断开后继续播放。rotate 生成并仅返回一次新 grant；revoke 写入没有对应 raw grant 的 fresh random locked digest。code-only policy 和密码保持独立；其变更只影响以后 code-only admission，除非 Host 明确执行强撤销。服务端永不返回现有 grant 明文；丢失 Host 标签页中的 grant 时只能 rotate。

房间号控件在复制按钮左侧提供换房 icon。它携带 exact Host token 与当前 creation profile 请求一个保证不同的新房间；服务端先完成新密码 material 与凭据生成，再以一个 RoomStore transaction 提交新 room 并删除旧 room，随后复用正常 abandon lifecycle 清除旧 session、route 与 SFU resource。提交前失败保持旧房不变；成功后活动分享结束、所有旧访问失效，客户端写入新 ownership、邀请和无期限 preferred code，不迁移旧媒体或自动重开 capture。
...
下表描述 accepted Browser 的单一 `screener-v13` wire；server 与 Browser assets 必须作为一个完整合同原子发布，其他 wire 在取得房间 authority 前拒绝，不存在旧 parser、writer、translator 或中间协议子集：
...
accepted 的唯一 wire 是 `screener-v13`，不做版本协商、旧 parser、双写或 translator。任何其他 Browser wire 及 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果。已通过 site access 的 well-formed code-only attempt 在没有 current room 时发 `ROOM_NOT_FOUND`；现存房间的 absent/wrong password、room full 与其他已知有界 admission refusal 继续发 `ROOM_ACCESS_DENIED`。两者都使用共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
2. 只有捕获 track 已存在时才请求创建或续租房间、连接 Host WebSocket 并显示邀请。新建请求携带本地 creation profile 和 preferred room，使 code-only policy、可选密码 verifier 与号码分配在房间可见前原子生效；偏好号码已占用时服务端直接随机分配并返回新的本地偏好。若用户在响应返回前取消，迟到结果不得覆盖后续分享会话，尚未认领的房间通过内部 `abandon-room` 立即释放号码。已经认领的房间在停止捕获后进入服务端 dormant lease，过期才释放；本地 preferred room 不参与该 lease。
...
浏览器可返回更低设置，界面以 `track.getSettings()` 显示实际捕获结果。推荐区始终只有表中的三档并默认选择中档 `1080p30`；高级分辨率额外提供 `480p`（854x480）、720p、1080p 与 1440p，FPS 继续独立接受 15 至 60 整数，码率继续独立接受 2 至 12 Mbps，因此不建立第四个推荐档或 `480p` preset ID。视频 track 使用标准 `contentHint = "motion"` 表达游戏动态意图；返回的音频 track 使用 `contentHint = "music"`。默认 `1080p30` 与折叠高级区初始值采用 `degradationPreference = "balanced"`，并允许显式选择清晰、均衡或流畅。高级设置在未分享时提供 `VP8 | 自动 | H264`，分享期间锁定；屏幕音频提供离散的 `64 kbps 普通 | 128 kbps 音乐默认 | 192 kbps 保真` sender ceiling。音频 ceiling 可在直播中串行更新。这些设置只是 ceiling 和浏览器偏好，不保证实际分辨率、帧率、码率或声道。
...
Accepted `screener-v13` Web 视频使用页面内 share-generation-scoped H.264/VP8 决策。Host 高级设置在未分享时按从低到高显示 `VP8 | 自动 | H264`，默认自动，分享开始后锁定。自动模式在取得 room/route authority 前，以一对无 ICE server 的本地标准 `RTCPeerConnection` 运行一个有界 H.264-only sender probe；手动 VP8 或 H264 跳过 gate 并严格使用所选 primary codec。Probe 使用按当前分享 effective target dimensions/frame rate 生成的短时确定性运动 Canvas track，并以 `motion` 表达同一内容意图；真实 capture/upstream track 只提供目标规格，其当前内容是否变化不参与 encoder capability 判断。Gate 只用实际协商 codec、同一身份的 probe media-source/encoded progress 与 bounded completion 判断，不读取 UA/GPU/MFT allowlist，也不单独相信 capability 或 `powerEfficientEncoder`；标准 Canvas capture 不可用、失败或 inconclusive 时使用 VP8。自动成功让后续 offer 按浏览器原生 H.264 capability 顺序列出 H.264，再列 VP8 与 repair formats；自动失败或手动 VP8 只列 VP8，手动 H264 只列 H.264 mode 1。Viewer relay 始终自动，并在该分享代首次收到 upstream video 且没有 active/prepared child 时运行同一 gate；若 child prepare 先到，则等待已经在运行的 bounded gate 结束后只创建一份真实 child sender。质量设置、换源、换父和分享暂停/继续不重复决定；只有停止后开始新的分享代才按当前 selector 重新判断。Viewer 原生播放器暂停不改变 remote track 或 relay gate。无法应用 codec preferences 或缺少所选 codec 时明确失败。SFU publication 使用 Host share generation 的同一 resolved H.264/VP8 决定并关闭 backup codec。所有新 Web peer sender 与 SFU publication 保留 `motion` 和用户选择的分辨率、帧率及码率 ceiling；当 desired degradation preference 允许分辨率下降时，启动阶段的 effective preference 暂为 `maintain-resolution`，当前 sender 累计编码至少五帧后由既有 stats 路径一次性应用 desired preference。该切换使用标准 `setParameters()` 进入或离开 balanced 时的原生 restriction reset，不增加周期重写、质量评分或应用层调档器。Host selector 只在页面内存，resolved 结果也不持久化、不进入 quality state/wire、不改写已连接 edge；实际 outbound codec/profile、encoder implementation 与 stats 是只读结果真相。应用 SDP 变换边界仍只有 Viewer answer 的 Opus stereo receive preference。
...
直播中切换质量不重新打开选源器：对当前视频 track 调用 `applyConstraints()`，再更新所有现有 sender 的参数；quality mutation 不重新运行 codec gate，新加入或重建的 Browser sender 使用当前分享代次的内存结果与最新其他权威设置。失败时保留当前分享并给出状态，不以重建整个房间补偿。临时暂停禁用当前 capture stream 的全部音视频 tracks；Host 通过当前 session 与 `shareGeneration` 发送一个 paused boolean，服务端同步 `host-status`、route pause 与 `authenticated` 快照，Viewer 在舞台显示“分享者已暂停”。Resume 使用同一状态更新并重新启用同一批 tracks，不另设 authorization、source acknowledgement 或 proof。晚加入和信令重连从快照恢复，恢复、停止、新分享或会话替换清除旧状态；不得把 track mute、黑帧或网络停顿推断成主动暂停。房间与连接保持不变。
...
codec 与 content-hint 依据由研究文档持有。`motion` 会改变 Chromium/libwebrtc 的适配取舍并可能空间降级以保留游戏动态；它是标准内容意图，不是分辨率或帧率保证。产品不再用 unset hint 固定 screen-classified 行为，也不围绕某次测量实现应用缩放器。Chrome 151 的普通 AMD H.264 MFT 即使报告 hardware efficiency 仍持续丢帧，而 Chrome 153 的 NVIDIA H.264 MFT、双 sender、Browser relay 与 LiveKit publication 均维持源 cadence，证明 capability、实现名称或单一硬件标志都不能替代实际 source/encoded progress。网页不启用浏览器进程开关或选择具体 GPU/MFT；gate 只选择 H.264-first 或 VP8-only 的标准协商输入，不控制实际 encoder。VP8 hardware acceleration 仍是证据特定事实；本设计不增加 VP9/AV1/H.265、自定义编码路径或转码。

`track.getSettings()` 中存在的 EC/NS/AGC/voice-isolation、`channelCount` 和 `sampleRate` 只能作为当前捕获观测；缺失字段保持未知，Opus 协商仍是独立事实。每个 Web Host 或浏览器 relay 的实际音频 sender 在首次绑定和换源后通过标准 `RTCRtpSender.setParameters()` 请求并读回当前 96/128/192 kbps ceiling；这只是上限，浏览器 WebRTC 拥塞控制仍可降低实际流量，不新增应用层音频降级器。96 kbps 普通是默认 ceiling；128 kbps 音乐和 192 kbps 保真由用户显式选择，均不是最低码率或质量保证。Peer answer 声明 `stereo=1;maxaveragebitrate=192000`，让同一接收协商覆盖所有三档，实际 sender ceiling 仍由所选档位约束；普通 PC synthetic 不用于标定参数、听感或性能。

Web peer 的最小 stereo 契约只改 Viewer 生成的 answer。实现把 MIT `sdp-transform` 2.15.0 与 `@types/sdp-transform` 声明为 root direct dependencies，不依赖 LiveKit 的 transitive install，也不用字符串或正则拼接 SDP。Helper 仅在解析出唯一非 rejected audio media、通过结构化 `rtp` entry 找到唯一 Opus payload 时，才在该 payload 的 fmtp 幂等 upsert `stereo=1` 与 `maxaveragebitrate=192000`；随后 `ViewerPeer` 把同一个结果交给 `setLocalDescription()` 和 signaling。首次连接、ICE restart、同 parent rebuild与浏览器 relay child 都走该边界。空 SDP、无或歧义 Opus、多个有效 audio、重复目标参数、畸形 fmtp、parse/write 异常都按对象 identity 返回浏览器原 answer，保住仍可继续的 mono/video edge，不发送半修改 SDP。RFC 7587 说明这两个值都是可选的单向 receiver parameters；`maxaveragebitrate` 是 maximum 而非最低保证，实际发送仍可随拥塞下降。当前 libwebrtc 把缺省 full-band stereo codec bitrate 配成 64 kbps，而 `RTCRtpSender.maxBitrate` 只限制上界；本接收上限允许三档，实际发送预算仍由 96/128/192 kbps sender ceiling 与拥塞共同决定。`sprop-stereo` 只是 sender likelihood hint，`usedtx` 缺省已是 0，本 slice 不增加二者，也不改浏览器协商的 `useinbandfec`。

SFU publisher 将 96 kbps 映射到 `AudioPresets.musicHighQuality`、128 kbps 映射到 `musicHighQualityStereo`，192 kbps 使用显式 `maxBitrate`；三者都使用 `forceStereo: true`、`dtx: false` 与 `red: false`。RED 与 Opus in-band FEC 是不同修复机制；关闭 RED 降低冗余流量并接受其弱网恢复退让，不声称禁用或替代 Opus FEC。直播中只改变档位时，不重新协商 Opus 或 republish；服务端沿现有 quality wire 保存 last-wins desired profile，各客户端 sender-mutation queue 从 fresh parameters 更新当前 P2P、relay 和 SFU audio sender并立即 readback，过期 completion 不覆盖新值，未来 sender 读取最新 desired profile。应用失败保留该 sender 的旧 applied 值和媒体并显示告警；无远端 applied 回执时 Host 不宣称全房原子收敛。Viewer 继续在同一个持久 `MediaStream`/媒体元素中增删 screen video/audio track；不新建 audio element、Web Audio mix/resample graph、第二 audio representation 或独立音频时钟。详细证据见 `docs/research/browser-screen-audio-quality.md`。
...
质量自适应采用 ADR-0007 的框架所有权，而不是全房间按最差链路降档。Direct/peer 保留每条 PeerConnection 的 stock WebRTC GCC。Browser Host 的唯一 SFU publication 不手写层列表，由锁定的 LiveKit client 默认行为拥有所选 H.264 或 VP8 的 screen-share representations、source replacement 与 republish。LiveKit Dynacast 和启用 send-side BWE 的 server stream allocator 独立处理聚合需求与每个 subscriber 的实际下行层，订阅端最高质量请求只是上限。Screener 不实现质量评分、带宽估计、周期调档或应用媒体层 selector。当前任一 Viewer 都可能承担 peer children，因此 subscriber 统一保持 `adaptiveStream: false`，relay ingress 不跟随本地 DOM 可见性。Pinned gate 证明了 LiveKit 原生空间选择和 Dynacast 能力，但具体 representation 参数仍属于 pinned component，而不是 Screener 产品常量。
...
Viewer 的已鉴权质量证据只描述当前路径，不能命令 WebRTC/LiveKit 改层、改变 Host publication 或改变拓扑。UA、平台和设备型号不参与质量或 relay capacity 决策。

SVC 的标准能力门禁已经静态关闭为 `no-go-web-svc-cross-path-hardware-contract`。WebRTC-SVC 只配置单个 outgoing sender，direct/peer receiver 没有对应的按层选择 API，独立 PeerConnection 之间也没有 shared-encode 契约；requested/applied codec/`scalabilityMode`、Media Capabilities `powerEfficient` 与 RTCStats encoder 字段不能证明编码器一定是硬件。Pinned LiveKit 可在 SFU 内为每个 subscriber 设置最大空间/时间层请求，但实际选择依 codec、SDK 默认和 server selector 而异，且不延伸到 direct/peer descendants。未来只能由能明确锁定硬编、跨 direct/peer/SFU 复用合格输出且通过游戏性能/延迟门槛的 native sender 以新决策重开。普通非分层编码包不能只靠转发生成另一档画质，实现路径差异只能由届时接受的 scalable representation 或转码模型拥有。
...
标准表示能力排在任何自定义媒体实现之前。Web P2P simulcast 缺少 receiver 选 RID 与跨 PeerConnection shared-encode 契约；LiveKit SFU 则原生拥有 per-subscriber 层选择与 Dynacast。ADR-0007 的 pinned-stack 门禁已通过，Browser SFU 的接受目标是锁定 SDK 默认的两路 screen-share 表示、Dynacast 和 server send-side BWE，并完全委托框架启停和选择；不得把该能力扩散到 direct/peer，也不得绕过 stock WebRTC congestion control。
...
- `/r/{code}` 观看者：首次解析合法 fragment grant 后立即清理地址栏并直达；仅有 code 时先通过 site access，再按 `open/private` policy 进入。私密房间统一返回通用拒绝并允许尝试密码，不披露房间是否配置了密码。鉴权后主区域最大化远端视频，标题旁显示可复制的紧凑房间号，顶部保留连接状态；同一个 video element 使用浏览器原生 controls 处理本地播放、音量和全屏，页面只保留“重新连接媒体”。Host 主动暂停时复用视频舞台遮罩显示“分享者已暂停”，恢复后立即移除。视频舞台用单一派生 presentation reducer 合成 access、Host presence、route prepare/active、peer/SFU connection、remote track、首个 composited frame、autoplay rejection 和 bounded failure。支持时用 `requestVideoFrameCallback()` 确认第一帧已提交，旧浏览器回退到当前代 decoded-frame delta 与 video readiness；`playing`、track object 或 ICE `connected` 单独都不足以结束占位层。当前代 composited frame 清除同 revision 的旧 connecting/reconnecting 展示。Hidden/freeze/pagehide 暂停应用 decoded-stall authority；visible/resume/pageshow 先 rebaseline 并重建 frame proof。系统暂停后的播放恢复留给真机和既有原生 controls，不猜测用户播放意图。Exact failure 在同步发送失败或信令换代后释放已报告锁，允许同一权威 route 重试。手动重连只重建同一 P2P parent 或同一 SFU subscription，不主动重选线路。
- SFU subscriber 以 LiveKit `disconnectOnPageLeave=false` 避免 SDK 在 BFCache `pagehide`/`beforeunload` 主动拆线；Screener 的 generation invalidate、显式 `disconnect(false)` 和服务端 Viewer grace 仍拥有正常清理。Pinned SDK 无选项关闭的 `freeze -> disconnect` 不做 fork，只进入真机 gate。
...
观看端不维护第二套音量、静音、播放或全屏状态；浏览器原生 video controls 是这些本地动作的唯一 owner。它们不进入 WebRTC sender、信令、房间状态或应用持久化，也不作为音频码率、声道、采样率或来源隔离设置。
...
- RoomStore 的轻量/SQLite 稳定双模式等价行为、exact schema/application identity/单 owner fail-fast、稳定重启只恢复 authority、crash-active 转 dormant、旧 revoked grant 仍拒绝，以及 `ROOM_LEASE_SECONDS` 默认/边界与固定 9000 房间码池；`ROOM_TTL_SECONDS` 继续 fail-fast；
...
- Production 与 canonical source 当前使用严格单一 `screener-v12` wire；quality-shadow 集成边界会原子升级到 accepted `screener-v13`。每分享代 actual-sender gate 在证据通过时让未来 direct/relay 和 Host 单一 SFU publication 使用 H.264，否则使用 VP8。本地 pre-share `VP8 | 自动 | H264` selector 已实现并在分享期间锁定；quality-state/wire、自定义 SFU layer list、backup publication与健康 edge codec 切换不存在。Browser SFU 使用 pinned LiveKit 默认 representations、Dynacast、server send-side BWE、disabled AdaptiveStream 与 RED off；精确执行状态由 `docs/status.md` 持有。
...
`reconcileRoom` 在没有 pending operation 时先处理 foreground route demand，再处理低优先级 direct convergence。Candidate planning 拒绝 stale session、不可 source-reachable、成环或没有稳态/overlap producer slot 的 candidate；排序依次使用结果深度、剩余 sender slots、稳定 child-parent hash、加入顺序与 peer identity。每次只 reserve/prepare 一个 exact candidate；WebRTC 负责 ICE/DTLS/consent，candidate child 解码出第一帧新画面后才同步提交资源和 graph。失败只消费该 exact tuple，健康旧边保持 sticky，不做周期性整树平衡或独立 depth hard cap。完整 cursor、bootstrap 与终态所有权由 ADR-0005 定义。

Pending candidate 的 receive-transport owner 只在当前 exact generation 下运行一个短生命周期 `framesDecoded` 观察器。全新 peer/SFU activation 以正累计值证明首帧；暂停期间仍保留的 prepared SFU 在 Resume 时先取 baseline，再要求后续增长。观察器在 proof、promotion、pause、换代或 teardown 时停止，不增加另一 success/failure deadline，也不参与 candidate 选择。Active route 仍由既有两秒 stats sampler维护诊断和 decoded stall，两者不得互相替代。SFU subscriber 的同一 sampler 从 active 开始持续到 deactivate/teardown；没有 video track、合并 report 为空或读取异常时只向 decoded-stall owner 交付一次 `null` progress sample，真实 metrics 仍只在有效 report 存在时交付。Track 换代同时换代 accumulator，旧异步结果不得驱动当前 route。

一个 operation 只有一个 timer 和一个总 deadline。Initial acquisition 给 direct 一个由生产首帧数据标定的固定 foreground window；显式硬失败可在该绝对窗口内推进其他 direct，silent candidate 到达边界后释放 reservation并转入SFU。Exact direct若在边界前报告标准WebRTC transport connected，就保留到总deadline继续等待第一解码帧；该进度不提交graph。SFU取得首帧后，剩余direct tuple才作为background逐Viewer轮转；没有SFU时仍使用完整deadline。不存在逐candidate timer或随人数切分的预算。

首次需要 SFU 但 Host 已满时，controller保存原始demand/session和本fact已试carrier集合，依稳定顺序让一个健康Host-direct child用普通SFU candidate建立publication。该candidate必须持有既有`TransitionOverlapSlot`；需要先切断健康carrier的bounded-gap计划不具资格。失败保持carrier及其subtree并尝试下一carrier；全部耗尽时终态归原始demand，同一fact不为后续demand重复整套carrier。成功后原demand下一次先尝试SFU，失败仍可继续direct suffix；carrier自身也进入低优先级direct convergence，待容量允许时恢复原Host直连。

所有非服务器端点共享一个 server-authoritative steady outbound media-copy cap `C`：唯一配置 `ENDPOINT_MEDIA_COPY_CAPACITY` 默认 `2`，只接受 `1`、`2` 或 `3`，不兼容旧键。服务端将 endpoint 当前广告的 `0..C` 可用值 clamp 为唯一 `effectiveDownstreamCapacity`；该值只表示当前可用 sender slots，不建立角色、浏览器、UA、设备或页面可见性 tier。peer child 或 Host publication 各占一份，upstream receive 免费；SFU subscriber egress 使用独立 server admission。Browser `screener-v13` 的 `authenticated.endpointMediaCopyCapacity` 把部署 `C` 交给 Host 与 Viewer；两端用该值校验稳态 assignment，并让 active+pending 的物理 copy 只在 controller 已授权 overlap 时达到 `min(C + 1, 3)`。代码通过一个 shared accounting guard 执行，不重复 magic number；其他 wire 与 Native executable 在 authority 前拒绝，不保留兼容路径。任何会产生第四份 endpoint media copy 的 token都不得签发；`C=3`满载bootstrap保持健康树并明确失败。

SFU admission 在应用单进程中只有一个注入实例。LiveKit credential tuple 完整时必须显式配置私有 `LIVEKIT_API_URL`；固定四位房号池给出 9000 个 publication ingress，上界 `9000 * MAX_VIEWERS_PER_ROOM` 给出 subscription egress，不存在独立 SFU 容量配置。专用 LiveKit 实例设置 `room.auto_create: false`。账本只保存全局已用计数，以及 exact room/share/publication generation 下的一份 publication handle 和按 Viewer identity 索引的 subscription handle。Publication 的 `reserved | committed | draining` 占 ingress `1`，每份 subscription 的同类状态占 egress `1`；离开 route 的 subscription 在该 generation 删除并确认 absent 前保持 draining，同一 Viewer handle 再次启用不重复计数。首个 SFU child 先 reserve publication 与自己的 subscription，再通过 `RoomService.CreateRoom` 创建 exact-generation room，成功后才 prepare 和签发 token；后续 child 只在同 generation reserve subscription 并复用 Host publication。exact candidate child 的 first decoded frame 后 commit 自己的资源；publication replacement 将旧 generation 及其全部 subscription 一起转入 draining，失败、超时、失联、分享换代、停止或删房按资源所有权进入同一生命周期。只有 `RoomService.DeleteRoom` 成功且 follow-up lookup 证明 room 不存在，账本才扣除该 generation 的 ingress/egress；auto-create 关闭后，旧 token 不能重建已删除 room。进程启动在接受流量前拒绝任何非 Screener room，删除全部遗留 Screener room并确认 namespace 为空；关闭时先 drain 再遗忘计数。Host 信令离线期间若 exact room 中的 Host participant 仍存在就继续计费并保留健康媒体，participant 消失后在一个有界轮询周期内回收。稳态 reserve/commit/drain bookkeeping 仍是 Map lookup 与整数加减；只有 generation 创建、删除、启动清场和 Host 离线检查访问 LiveKit control plane。多应用进程部署必须先增加共享原子 admission/lifecycle owner。
