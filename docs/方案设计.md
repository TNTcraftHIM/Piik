...
本地一键启动只包装当前 Node/Vite 源码应用，不创建第二套服务器。`npm run local` 和 PowerShell/POSIX wrappers 检查 Node 24 与依赖、选择可用端口、使用专用本地状态目录、生成或复用符合当前合同的 `SITE_ACCESS_PASSWORD`，启动后轮询现有 health endpoint，并打印 localhost Host URL 与 LAN Viewer URL。它明确报告 HTTP LAN、缺少公网 TLS、防火墙/端口映射和未配置 STUN/SFU/TURN 的边界；不自动修改系统网络设置，不把源码启动器称为安装包或互联网部署器。测试通过注入端口、网络接口和子进程边界验证配置与输出，不依赖真实公网。
...
7. 加入 first-frame-driven Viewer connection stages，再整合 source-checkout LAN launcher。Site access、room-scoped Viewer grant 与 SQLite v3 的已部署合同保持不变。
8. Windows Native fixed-`HIGH` 保持可选且不改变 Web 默认；shared encode、移动 lifecycle、可分发 sender 和平台输出按 TODO roadmap 后续推进。
