...
状态：current source 精确 `f5a295c` 使用严格单一 `screener-v11`；Browser display video 不设置 `contentHint`、audio 保留 `music`，唯一视频媒体 codec 为 VP8，UI、quality state/wire 和分享生命周期均没有 codec 分支。Production 精确 `2726edde9b87f31fd76e749de47972ef817a9bd5`、release `2726edd`，仍使用 `screener-v10`、设置 video `contentHint = "motion"` 并保留 pre-share codec selector；`screener-v11` 尚未部署。Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。
...
下表描述 current Browser 的单一 `screener-v11` wire；server 与 Browser assets 必须作为一个完整合同原子发布，其他 wire 在取得房间 authority 前拒绝，不存在旧 parser、writer、translator 或中间协议子集：
...
current 的唯一 wire 是 `screener-v11`，不做版本协商、旧 parser、双写或 translator。任何其他 Browser wire 及 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
- Current source 精确 `f5a295c` 使用严格单一 `screener-v11` wire，Browser 固定 VP8 且 video 不设置 `contentHint`；codec selector、quality-state/wire 字段与分享生命周期 codec 分支不存在。Production 精确 `2726edde9b87f31fd76e749de47972ef817a9bd5`、release `2726edd`，仍使用 `screener-v10`、设置 video `motion` 并保留 pre-share codec selector；`screener-v11` 尚未部署。
