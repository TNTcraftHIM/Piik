...
状态：Current source 与 production 使用单一 `screener-v10` Browser wire，默认 VP8，codec selector 仅在分享前用于诊断且结果在 share lifetime 内固定；Pause/Resume 由当前 Host session 和 `shareGeneration` 约束。精确 deployed behavior 只由 deployment 文档记录。
...
Viewer grant 与 code-only 入口是两个正交维度。每个房间都用 `randomBytes(32)` 生成含 256-bit 随机量和到期时间的 room-scoped opaque grant，服务端只在内存保存完整 grant 的 SHA-256 摘要并用定长比较验证。当前 grant 从建房或 rotate 起固定有效七天；房间删除、进程重启或 revoke 会使它更早失效。Host 的主分享动作始终复制 token-bearing invitation；到期在以后鉴权/重连时拒绝，即时清退使用 rotate/revoke。
...
- Current source 与 production 使用单一 `screener-v10` wire，默认 VP8，codec selector 只在分享前可用且对本次分享固定，Pause/Resume 由当前 Host session 和 `shareGeneration` 约束。
