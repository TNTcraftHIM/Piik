...
显示名是独立的本地偏好：一个同源 `localStorage` 值供 Host 与 Viewer 共用，未设置时使用角色默认标签。客户端鉴权时发送经同一严格 schema 校验的纯文本名称，服务端只在当前在线房间内存中保存；每次加入、离开或会话替换后广播不超过一名 Host 加配置观看上限的完整权威 roster。展示标签使用名称加当前房间服务端 `peerId` 的至少六字符短后缀，房内碰撞时延长；不暴露 `clientId`，也不伪称全局 UUID。名称、短后缀和 roster 都不替代 role、Host token、Viewer grant、peer/session ID 或任何授权/路由判断，SQLite 不保存它们。该 UI/roster 实现保持为独立后续 PR，不混入授权迁移。
...
| 方向 | 消息 | 作用 |
| --- | --- | --- |
| client -> server | `authenticate` | Host 以 upgrade 时的 admission 状态加 room-scoped Host token 建立会话；Viewer 以 public room code 或 private Viewer grant 建立仅观看会话 |
