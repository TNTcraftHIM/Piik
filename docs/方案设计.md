...
状态：首个 PoC 基线及其已接受扩展的当前设计。Production 仍使用 STUN-only ordinary ICE，并只为 room `1` 启用 exact-room peer-assisted 与 LiveKit SFU/UDP。默认关闭的 built-in Peer ICE TURN canary 已失败并回滚，不再是产品方向；下一候选只能在 SFU/UDP 后由 controller 选择单条异常 edge。旧 release 与 coturn relay 仅保留为隔离回滚路径，不是同进程兼容分支。
...
当前仓库已经删除旧全房 TURN contract，`ServerConfig` 必须配置 `STUN_URLS`。
Source 仍含 `PEER_ICE_TURN_*`、participant capability、`refresh-ice` 与
`ice-config` 的 default-off built-in canary；其真实 direct acceptance 失败并完整
回滚，因此这些字段不得配置、部署或作为 accepted target。后续 selected-edge
实现应以新的完整 default-off tuple 替换这组 participant-wide contract，部分配置、
非法 UDP TURN URI、弱或复用 secret、越界 TTL 仍必须启动失败。旧 `TURN_URLS`、
`TURN_SHARED_SECRET`、`TURN_CREDENTIAL_TTL_SECONDS` 继续 fail-fast，绝不作为 alias。
...
与 parent session、route revision、replaced connection 和 server-generated next
connection identity；parent 与 child 只为该新 PC 协同重建，Viewer 使用 relay-only
配置。任何代次变化、超时、成功或失败都消费内存态 grant；普通 PC、其他 edge、
Native v2 与 authenticated snapshot 继续 STUN-only。凭据不进入 URL、日志、browser
...
当前可执行梯级是 `direct/peer UDP -> SFU ICE/UDP -> failure`；accepted target 在
SFU/UDP 后增加一次 selected-edge TURN，再失败则明确终止。Viewer 的
...
`PEER_ASSISTED_ROOM_IDS` 灰度 topology/SFU route；未命中房间始终 STUN-only，且
普通 peer edge 即使命中也不能默认收到 TURN candidates。因此验证与回滚必须使用
...
edge 或 generation；应用层必须在签发、parent/child rebuild、ready/failure 与清理
...
- tracked `stun-only` 及 `no-tcp`/`no-tls` 配置只是当前回滚/验收前模板，不是最终产品目标。当前共享机的应用 TURN tuple 已关闭且只下发 STUN，daemon/防火墙仍是旧 authenticated-relay baseline（TCP/UDP 3478 与 UDP 49152-49251），没有 credential 下发，最后审计 allocation 为零。Selected-edge 验收必须使用独立 realm/secret、小 relay UDP range 与 allocation/bandwidth quotas，并先验证 STUN、SFU/UDP exhaustion 与单条短期 credential；不启用匿名 TURN 或媒体 TLS/TCP。coturn 4.17.2 已弃用 `no-dtls`，tracked 临时配置不使用它。
...
6. 在 SFU/UDP retained-media gate 后增加默认关闭的 selected-edge TURN tuple、revision-bound one-use grant 与 parent/child rebuild；用有界网络矩阵验收 ordinary STUN-only、SFU/UDP、单条 forced relay、资源成本和明确失败，coturn/production rollout 保持独立验收边界。
...
bounded controller，且 production 已启用 room `1` exact smoke；下列内容保留为
...
不接收 TURN config。命中房间的 ordinary peer PC 与 Native v2 同样保持 STUN-only；
只有 controller-selected edge 可接收一次短期 grant。该边界不增加 UI、用户选择或百分比框架。

产品硬目标是任一非服务器节点任何时刻最多两条下游媒体边。系统按一个或两个观看者直接 P2P、更多观看者 peer-assisted、部署 SFU 中央兜底、最后 selected-edge TURN 的顺序自动分配和恢复，分享者与观看者不选择拓扑。默认媒体 transport 只使用 UDP；SFU 作为 virtual parent 正常只送一至两个 roots，roots 之后仍优先沿受限 peer descendants 扩散；多个无法挂到健康 root 的极端 viewers 只可使用另设上限的中央 edge。Production 已把普通 ICE 收敛为 STUN-only、删除旧全房 TURN wire并只为 room `1` 开启 failure-only controller；source 中失败的 optional built-in Peer ICE TURN 仍默认关闭且不得部署，replacement selected-edge 尚未实现。exact-room 公网 SFU media、selected relay 与资源仍未验收，旧 release/coturn 仅作回滚。ADR-0004/0005 的 broad-rollout 验收尚未完成。
