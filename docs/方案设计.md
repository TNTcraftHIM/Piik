...
状态：已接受的下一 Browser 合同固定 VP8 并移除 codec selector/wire 字段，以单一 `screener-v11` 原子替换 v10；Current source 与 production 在该实现合并和部署前仍是 `screener-v10`。Pause/Resume 继续由当前 Host session 和 `shareGeneration` 约束。精确 deployed behavior 只由 deployment 文档记录。
...
下表描述 accepted Browser 的单一 `screener-v11` wire；server 与 Browser assets 必须作为一个完整合同原子发布，v10 和更早客户端在取得房间 authority 前拒绝，不存在旧 parser、writer、translator 或中间协议子集：
...
accepted 的唯一 wire 是 `screener-v11`，不做版本协商、v10 parser、双写或 translator。任何 v10 或更早页面及 executable sender 的首条消息在角色进入 room lookup 前得到同一个 fatal refresh 终态并停止自动重连。Client 与 server 共用 `SIGNAL_CLOSE_CODES` 定义会话替换、鉴权失败和 Viewer access revoke 的终态语义，避免双方对重连边界产生分叉；非法协议消息使用 WebSocket policy-violation close code。缺失、过期、跨房、已 rotate 或 revoked 的当前 Viewer grant 使用同一个通用“邀请无效或已失效”结果，不区分房间不存在与凭据错误。已通过 site access 的 well-formed code-only 预期拒绝只发 `ROOM_ACCESS_DENIED`：unknown/expired room、disabled policy、absent/wrong password、room full 与其他已知有界 admission refusal 使用同一 code、公开文案和共享 authentication-failed close code。Grant 继续使用 `INVALID_TOKEN`，Host 鉴权保持独立，unexpected internal failure 使用不含 raw message 的 `SERVER_ERROR`。
...
Web 视频端固定使用 `VP8`。P2P 与 browser relay 在首个 offer 前只保留 VP8 及其浏览器公布的 repair codecs，SFU publication 显式使用 VP8 并保持 `backupCodec: false`；无法采用 VP8 时明确失败，不静默协商另一种产品 codec。质量设置、房间 state、Pause/Resume、换源和 sender mutation 都不保存或改变 codec。实际 outbound codec/profile、encoder implementation 与 stats 仍是只读结果真相。Viewer answer 的 Opus stereo receive preference 是当前唯一应用 SDP 变换边界。
...
固定 VP8 的依据由研究文档持有：同机 Chrome 的 Windows minimum-power GPU preference 选择 AMD H.264 MFT并稳定低帧，Edge 选择 NVIDIA MFT 后恢复，而网页没有选择 GPU adapter/MFT 的标准 API；H.264 profile 重排和 Chromium high-performance GPU flag 都不能修复 Chrome 的 MF encoder 选择。该结论不把合成 loopback 当作游戏画质排名，而是拒绝一个依赖宿主系统配置、应用无法约束的固定 Browser codec。实际 codec 与 encoder 字段继续用于诊断，不触发 codec 切换。
...
- 已接受的 Browser v11 固定 VP8 并删除 codec selector/wire 字段；H.264 不进入 browser relay 或 SFU 验证。Host 后台性能按受控根因证据诊断；标准 ICE/SFU 按真实异构网络验收。
