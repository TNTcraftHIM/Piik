...
- [Verification status](./verification-status.md): exact v9 source gates, current v8 production evidence, open physical proof boundaries, and expensive-test applicability.
...
- [Deployment](./deployment.md): exact v8 current/rollback production configuration, the not-yet-deployed v9 source contract, release operations, and transport evidence boundaries.
...
- [Peer-assisted media research](./research/peer-assisted-media.md): browser shared-encode limits, standard relay re-encoding, deterministic sticky topology, v9 exact-route implementation evidence, and remaining physical gates.
- [Low-server-cost media routes](./research/low-server-media-routes.md): Host-publication/SFU distribution, v9 route diagnostics, direct-to-SFU conclusion, hop-level accounting, privacy-safe ICE evidence, and bounded transport gates.
...
- [Realtime quality adaptation](./research/realtime-quality-adaptation.md): current v9 manual profile/codec boundaries, diagnostic-only capture/send/receive correlation, path-isolated dual representations, and bounded simulcast/Dynacast/SVC checks.
...
- [Browser screen-audio quality](./research/browser-screen-audio-quality.md): capture/source compatibility, v9 live sender-ceiling readback, Opus negotiation boundaries, and remaining audible/device evidence.
...
- [ADR-0005](./adr/0005-automatic-hybrid-media-routing.md): accepted P2P-first single-reconcile route model; its v8 baseline is deployed, while exact v9 first-frame/resource/lifecycle corrections are implemented but not yet deployed and remaining real-network validation is tracked in verification status.
