...
- Current source and accepted room admission support one Host plus up to `20` Viewers. The route release still requires one 20-Viewer smoke without changing endpoint or server-resource accounting.
...
- Automatic P2P-first routing, one active upstream, an acyclic source-reachable graph, exact Browser v7 prepare tuple, first-decoded-frame make-before-break, strict rollback revision, and bounded failure remain accepted.
- The accepted controller has one event-driven reconciliation loop and at most one room-serial child operation. That object owns one deterministic tuple list/cursor, one current media candidate and reservations, fact version, and one total deadline. Join, exact-edge failure, relay disconnect, and effective-capacity reduction all enqueue that operation. Failed tuples are operation-local; exhaustion retires invalid/overflow edges but retains healthy bootstrap media. Bounded-gap preflights server resources, replans the remaining suffix, and restores a healthy retired edge through one final ordinary candidate. Admission commits synchronously before graph promotion; release uses the resource-set difference and room disposal returns all resources. One Host publication serves exact Viewer subscription handles under the same SFU generation, and actual global SFU/TURN usage release wakes only registered waiting rooms. Endpoint accounting, v6 capability fencing, aggregate generation lifecycle, and TURN admission are in current source; v7 Web wire, exact subscription admission, decoded-progress stall, and runtime controller convergence remain pending.
...
1. Complete and release the audited generic route loop on the single Browser `screener-v7` wire with deterministic parent selection, one first-frame candidate transaction, event-driven SFU/TURN waiter wakeup, and efficient admission through 20 Viewers.
