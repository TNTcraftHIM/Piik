...
- `https://share.bonfire.icu` runs exact deployed application/runtime revision `b68c47167da592dd673a78ec3d072936ef49e3ea`, release `b68c471`, wire `screener-v12`, from `/opt/screener/releases/b68c471`; that revision is integrated in canonical `main`. Its immutable runtime ZIP SHA-256 is `e5932880677ebb745ef9325f8def2185d88cdd7bb557fada7c3e1a9e1a7c2ffd`, and its 38-file manifest SHA-256 is `12e092822abad0d8448d9c6ec87386e8ff82eaafecfeab766e050fa0f3b46710`.
- `screener`, LiveKit, coturn, and nginx are active/running; local/public `/healthz` return 200, all six served Browser files match the immutable release, and all four services report `NRestarts=0`. The main Browser asset is `assets/index-BSZhUco2.js` with SHA-256 `30cf3bea2a7f80c5b0140f0e40fb0f3f3c082a6195e2ad0990509c663a605f96`; the prior asset returns 404.
- The service process resolves its working directory to exact release `b68c471`; it has no writable room `StateDirectory` or SQLite runtime.
...
- Exact `main@b68c471` passed repository hygiene, TypeScript, all 609 Web tests in 44 files, both production builds, the 261-test SFU gate, the explicit-Chrome access/privacy gate, focused independent review, and the production access/postflight checks above. Retained 20-Viewer loopback, responsive visual, and physical SFU evidence remains separately revision-bounded in [verification status](./verification-status.md).

## Current Milestone

1. Physically validate the corrected live screen-audio statistics and 64/128/256 kbps ceilings with known audible content across direct, browser-relay, and SFU paths.
2. Physically verify fixed-VP8/no-video-hint direct, browser-relay, and SFU paths from actual codec, capture, outbound, and decoded stats.
