...
- Canonical integration truth remains the root `main` branch. The current Browser v9 source checkpoint is exact `d543f38aacad3df5ef65fde1055cc8e733972afe`; auxiliary branches and older worktrees do not supersede it, and it is not the deployed release.
...
- Exact source `d543f38aacad3df5ef65fde1055cc8e733972afe` passed 670 Web tests in 48 files, TypeScript typecheck, client/server production builds, the access/privacy gate, production-dependency audit, and repository hygiene. Its exact one-Host/20-Viewer Chrome loopback passed every gate with all Viewers decoding and Host/relay fanout bounded by cap `2`; all observed media routes were local direct peers and no SFU publication appeared. This is source, state-machine, build, privacy, and direct-loopback evidence, not deployment, public-network, SFU, mobile, physical audio/codec, H.264 performance, or Host-background evidence.

## Current Milestone

1. Integrate and deploy Browser v9 as one atomic release; production stays on exact v8 until postflight proves the full checkpoint.
2. Validate codec switching and live audio ceilings on physical direct, peer-relay, and SFU media paths, including actual codec/bitrate, first-frame, source authority, rollback, audible quality, and A/V synchronization.
3. Reproduce the dated H.264/blur and Host background/minimized reports under controlled conditions, then finish representative ICE/STUN/SFU acceptance including mobile networks. Browser port prediction, NAT classification, TCP probing, fake page keepalive, and quality-driven reparenting remain outside the accepted model.
...
- Room lifetime, authorization/storage semantics, restart loss, and rollback restoration remain aligned. Source uses the v9 stale-client boundary and new room-entry presentation; production remains on the v8 boundary and v8 UI until atomic deployment.
...
There is no active P0/P1 source hold. Browser v9 is implemented and locally gated but not deployed;
production remains on exact v8 until the atomic release and postflight complete. Physical codec/audio,
heterogeneous-network, SFU, H.264/background, and mobile evidence remains open. Native/executable work
and broad repository cleanup remain outside this release boundary.
