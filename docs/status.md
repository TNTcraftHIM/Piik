...
- Automatic P2P-first routing, one active upstream, an acyclic source-reachable graph, exact prepare child/connection authority, first-decoded-frame make-before-break, and bounded failure remain accepted.
- The accepted controller has one event-driven reconciliation loop and at most one room-serial child operation. That object owns one deterministic candidate list/cursor, one current media candidate, its reservations, and one total deadline. Join, exact-edge failure, relay disconnect, and effective-capacity reduction all enqueue that operation. A relay repairs its own ingress as a child while retaining its subtree; excess direct children use the same operation before a disconnected childless relay is removed. One Host publication serves exact Viewer subscription handles under the same SFU generation, SFU-fed endpoints use the ordinary provisional-child path, and TURN changes edge transport rather than topology. Endpoint accounting, the wire boundary, aggregate generation lifecycle, and TURN admission are implemented; exact subscription admission and controller convergence remain pending.
