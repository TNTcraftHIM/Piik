...
- The source candidate adds click-only, versioned JSON export beside expanded connection details. A fixed allowlist excludes candidate addresses/ports, participant and room identity, raw signaling, URLs, and credentials; no report is uploaded or persisted. Production rollout is pending.
...
- Relay quality handling compares fresh child FPS with parent input. Two current corroborating children can pause one Viewer relay for 30 seconds and request peer-only reassignment. The source candidate keeps the triggering child's old edge while one ordinary peer-upstream Viewer proves current-generation RTP, decoded frames, and live video; v1 excludes Host/SFU roots, preserves ready-failure identity through commit/rollback, and lets unrelated hard failures preempt the soft probe. Failure, timeout, or no candidate keeps the old edge without SFU/TURN. Sibling MBB remains open.
