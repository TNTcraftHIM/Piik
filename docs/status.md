...
- `https://share.bonfire.icu` runs exact `aa870c51a57a51166a605c734cbb03263a72d12d`, release `aa870c51a57a`, wire `screener-v5`; `01a77ae331de` is the rollback release.
...
- A source candidate additionally shows the selected pair's local cumulative STUN responses and a bounded same-pair sample delta. It is observation-only, is not deployed, and does not trigger recovery or upload the pair identity.
