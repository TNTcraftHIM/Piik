...
- Production runs exact `01a77ae331ded864819003b532695470d7777e7d`, release `01a77ae331de`, with `93e468191576` retained for rollback. Its audited runtime tree has no regular-file inode shared with rollback, and all public client assets matched the exact local build byte-for-byte. The prior `93e4681` no-room Chrome 151 canary passed health, unauthenticated WSS open/close, STUN-only configuration, and `srflx` gathering in 2.49 seconds without logging candidate data; it was not rerun for `01a77ae`, and room media, route migration, SFU, TURN, and performance remain unverified.
