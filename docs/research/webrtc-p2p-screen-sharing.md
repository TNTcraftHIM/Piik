...
新的产品目标要求分享端 fanout 永远不超过二。Proposed ADR-0004 用一个可整体删除的实验验证第三名及后续 viewer 能否由客户端转发；Draft PR #12 则保留可选单节点 SFU 作为独立对照。自动跨模式兜底是产品要求，但当前两个 Draft 都未实现它；超过小房间上限时仍建议使用外部直播服务。观察项包括：
