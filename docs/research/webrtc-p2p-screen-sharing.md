...
首选顺序：一至两名观看者直接 P2P -> 通过门槛后的 peer-assisted -> 部署者显式选择的可选 SFU。
该顺序不允许静默拓扑迁移；大规模公开分享仍直接使用现有直播服务。
```
...
新的产品目标要求分享端 fanout 永远不超过二。Proposed ADR-0004 用一个可整体删除的实验验证第三名及后续 viewer 能否由客户端转发；Draft PR #12 则保留显式单节点 SFU 作为独立对照。两者都不允许在房间内静默切换，超过小房间上限时仍建议使用外部直播服务。观察项包括：
...
7. 可选 SFU 保持显式、可逆且非默认。peer-assisted 若未通过逐跳重编码负载、延迟、兼容和换父门槛就整体放弃，不通过 encoded custom media、评分器、多树或自研 RTP 来挽救。
