...
- C+B stays edge-local: deployed MBB v1 gives only the triggering child one ordinary peer-upstream Viewer attempt while its old edge remains active; Host/SFU roots remain excluded. Current-generation RTP, decoded frames, and live video atomically promote the candidate; a ready-then-failed identity survives until active/rollback, and an unrelated authoritative failure preempts the soft probe. Failure, timeout, or no candidate keeps the old edge without SFU/TURN. Sibling MBB remains separate. No score.
...
- Production: exact `66eb337171931edaf5d621600c3803a186cf85ac`, release `66eb33717193`, Web wire `screener-v5`; `d315d333e0a8` is the rollback release.
...
- Production rooms default to eight Viewers and accept an explicit admission limit from one through sixteen. Web endpoints use cap2 with one upstream; source supports an explicit deployment cap from one through three without a UA/visibility split.
...
- Chrome 151 local functional evidence includes five-Viewer cap2 and sixteen-Viewer cap2/cap3 runs. All Viewers decoded, and a ten-second follow-up recorded guarded sender and aggregate Chromium CPU evidence, but resolution/FPS stayed low and the same-machine ordinary-PC runs do not rank cap2 against cap3 or close resource, visual-quality, endurance, heterogeneous-network, or 20-viewer gates. Production remains cap2.
