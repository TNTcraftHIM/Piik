...
- ADR-0005 accepts direct-first Peer ICE and bounded SFU roots. Its first exact-room TURN canary failed the real direct acceptance path before forced relay and rolled back completely; production advertises STUN-only ICE. This implementation no-go does not reject authenticated per-viewer fallback.
...
- Production ICE is STUN-only. Source adds expiring STUN+TURN only for capable Web sessions in exact rooms; its first production canary is no-go and inactive. Other rooms/Native stay STUN-only. Room `1` may feed two LiveKit roots with peer descendants; real rescue/relay/SFU media and game quality remain unverified.
...
- Whether Peer ICE TURN can first pass direct Host/Viewer establishment, then forced relay, mobile, expiry/reconnect, 1/3/5/8 allocation/resource, 1/2 relay-bandwidth, and 1-GiB gates. The failed canary left no application tuple, advertised credential, transient rule, or allocation; the old coturn relay config and ports remain. Media TCP is out of scope.
