...
- The Web control plane is deployed at `https://share.bonfire.icu` behind nginx with Node.js 24.19.0, and `turn.bonfire.icu` is configured for authenticated coturn 4.17.2 on standard UDP/TCP 3478. HTTPS, WSS, room authentication, certificate renewal, and the coturn private listener are verified. TURN/TLS is intentionally not enabled.
...
- The immediate blocker is Tencent Cloud ingress: public TURN probes do not reach the VM despite correct guest listeners and firewall rules. The next milestone is to open TCP/UDP 3478 plus UDP 49152-49251, prove UDP and TCP relay candidates, and run the manual matrix in `docs/status.md`, not add product surface or a native sender.
