...
- Direct/peer keeps per-PC stock GCC. Active SFU explicitly disables Dynacast, publishes exactly `q,h`, and gives each subscribed root a `HIGH` ceiling; built-in BWE may still forward `LOW`. The source gate proves roots <=2, Host-publication edge accounting, and retention of a zero-child subscription. Actual shaped BWE and resource evidence is next; if it passes, no app media selector is built. Explicit quality, manual activation, and custom/native remain fallbacks.
...
- Production: exact `01a77ae331ded864819003b532695470d7777e7d`, release `01a77ae331de`, Web wire `screener-v5`; `93e468191576` is the rollback release.
...
- Production keeps ordinary ICE STUN-only, SFU roots <=2, selected-edge UDP TTL 120, and one `peer-selected` lease per room excluding Host ingress. The `01a77ae` release has an independent dependency tree, public client assets byte-identical to its local build, and healthy bounded services. The prior `93e4681` no-room browser self-check passed health, unauthenticated WSS open/close, and STUN `srflx` gathering without exposing candidate data; it was not rerun for `01a77ae`, and room media, SFU, TURN, and performance remain unverified. Earlier exact-`16f6eab` evidence proves active Host ingress only.
...
- Defaults are balanced with automatic codec and 128 kbps screen audio; clarity/fluid, H.264/VP8, and 64/128/256 kbps audio remain explicit. Codec and audio quality are locked during a share. LiveKit 2.22.0 SFU explicitly disables Dynacast and uses ordered active `q,h`; the source gate rejects root3, charges the Host publication edge, and retains a zero-child root/subscription. Production exposes one Host-local current-`h` A+B snapshot and clears it on publisher identity changes. Target-browser field availability, actual media values, BWE, and route observation remain open.
