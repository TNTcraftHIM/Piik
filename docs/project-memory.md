...
- Production: exact `aa870c51a57a51166a605c734cbb03263a72d12d`, release `aa870c51a57a`, Web wire `screener-v5`; `01a77ae331de` is the rollback release.
...
- Production keeps ordinary ICE STUN-only, SFU roots <=2, selected-edge UDP TTL 120, and one `peer-selected` lease per room excluding Host ingress. Release `aa870c51a57a` deploys the root invariant and `dynacast: false`; real shaped LiveKit media, BWE, resource, SFU, TURN, and broader performance evidence remain open. Release `01a77ae331de` is retained for rollback. Earlier exact-`16f6eab` evidence proves active Host ingress only.
