...
- Production `89e6d7649169e43276a604c36fe681ebbed384dc` deployed at 2026-08-20 09:40 +08 to `https://share.bonfire.icu`. It enables peer/SFU routing only for persistent room `1`; unlisted rooms remain ordinary P2P. Initial-connect recovery and bounded SFU diagnostics are active, while `bbe4654` is rollback-only.
...
- Production requires an independent `HOST_ADMISSION_PASSWORD` only for creation/Host role. Default private fragment grants and explicit public-watch authorize Viewers; signaling is role-bound and there are no accounts/JWT/session rows. SQLite v2 stores Host and nullable Viewer-grant digests; all four rooms, including room `1`, survived the stopped migration.
- Production uses nginx, Node.js 24.19.0, coturn 4.17.2 for STUN/rollback, and pinned LiveKit 1.13.5 on UDP 7882 with TCP fallback disabled. Screener and LiveKit report `NRestarts=0`; local/public health and `index-2HASsWrj.js` return 200, while `/rtc`, `/rtc/validate`, and unauthenticated `/rtc/v1` return 404 outside the SPA. Public TCP 7880/7881 is blocked; LiveKit has a 192/256 MiB cgroup boundary and no automatic restart.
