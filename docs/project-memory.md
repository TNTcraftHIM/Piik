...
- ADR-0006 stays no-go: the clarification run retained VP8 output and one first-generation binary `send()` return, then no post-send diagnostics and a sanitized fatal marker; media zeros are initial, no Viewer was created, and cleanup passed 5/5. Source now removes one deterministic timestamp-overlap fatal by deriving RTP cadence from increasing capture timestamps rather than nullable WebCodecs duration. Focused/handler Go tests pass, but the retained fatal category and post-fix Chrome/Viewer proof are absent. The MF RTX H.264 fixture passed 360/360, but `42c01f` misses Pion's default fmtp. Native v2 remains memory-only and leaves SQLite/Web unchanged.
