...
1. **Release the converged Browser route (held pending owner direction).** The one-controller `screener-v7` source is implemented, merged, typechecked, and focused-review clean. Start no release action until the owner resumes this item. Then run one resource-limited 20-Viewer Browser smoke that records route convergence, first-frame or bounded failure, endpoint sender counts, application CPU/memory, and SFU/TURN usage, followed by production preflight, deploy, postflight, and rollback verification. The existing Chrome attempt never reached a Browser/media run and must not be counted or retried unchanged; [verification status](./verification-status.md) owns that boundary. Do not repeat the full local suite or build/run Native, helper, shared-encode, Go test, or hardware executable artifacts.
...
1. **Implement memory-resident rooms and clear sharing controls.** Replace the two current room modes with one in-memory model: uniformly random free codes from `1000..9999`, `ROOM_LEASE_SECONDS=86400` by default, no expiry while Host sharing, dormant renewal only by the exact Host token, release after expiry, and total loss on process restart. Remove SQLite, `ROOM_DATABASE_PATH`, `ROOM_TTL_SECONDS`, schema/migration code, and compatibility surfaces atomically. Make Viewer grant independent of `open | password | disabled` code entry; default to `open` with no room password while the primary share action copies the token-bearing invitation. Persist the Host's display name, code-entry policy, and optional room password as a same-origin local creation profile and atomically reapply it to a newly created room after expiry/restart. Give room code, invitation link, grant rotation/revocation, and code/password policy distinct direct labels and familiar icons. Current source still implements the old model; do not report this item complete until source, tests, deployment guidance, and browser behavior agree.
