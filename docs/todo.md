...
1. **Deploy Browser v10 atomically.** Current source defaults new shares to VP8, keeps `automatic | H.264 | VP8` as a pre-share diagnostic choice fixed for that share, uses `shareGeneration`-fenced Pause/Resume, and exposes the single v10 wire. Complete integration, deploy server and Browser assets together, reject stale v9 before room authority, and record the exact release boundary.
2. **Classify H.264 in parallel and select one fixed Browser codec.** Reproduce the dated blur/performance report on the same Host, browser, driver, game scene, and wired direct Viewer while changing one variable at a time. Validate fresh shares on physical direct, browser-relay, and SFU paths, choose VP8 or H.264 from actual codec/profile/encoder and capture/sender/Viewer evidence, and make that codec the only Browser product choice.
