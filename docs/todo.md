...
1. **Remove the Browser video content hint.** Stop assigning any video `contentHint` so display capture keeps the browser's screen semantics; retain audio `contentHint = "music"`. Verify the focused quality helper and a real `getDisplayMedia()` encode sample. This source-only change does not alter the v10 wire or decide the final codec.
