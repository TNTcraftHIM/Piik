...
1. **Ship the fixed-VP8 Browser contract.** Remove the temporary `automatic | H.264 | VP8` UI, quality-state/wire field, and share-lifetime codec branches; use VP8 for direct, browser-relay, and SFU senders, bump the private Browser wire once, and deploy client/server atomically. Keep actual codec/profile/encoder stats as diagnostics. Do not add GPU selection, SDP munging, profile fallback, or H.264 relay/SFU work.
