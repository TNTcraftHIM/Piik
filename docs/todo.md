...
2. **Resolve H.264 and remove Browser codec choice.** Reproduce the dated blur/performance report on the same Host, browser, driver, game scene, and wired direct Viewer while changing one variable at a time. Implement a codec-specific repair only if actual codec/profile/encoder and capture/sender/Viewer evidence identifies one, then validate fresh shares on physical direct, browser-relay, and SFU paths. Choose VP8 or H.264 as the single fixed Browser codec from that evidence and delete `automatic | H.264 | VP8`, the selector, and the remaining codec-choice product code for either result; do not rebuild live codec switching.
