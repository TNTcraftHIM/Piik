...
The embedded-media phase is already integrated and deployed. The completed
detailed Debug supplement is now combined with this candidate for acceptance.
Its [configuration reference](./reference/configuration.md) owns activation,
bundle contents and remaining identifier disclosure. Verify the new pool's
operation and producer evidence through that same recorder, including explicit
partial/truncated export. No separate logger or diagnostic media authority.
Public release and piik branding/site work remain separate product stages.

The owner-confirmed phase order is:

1. Finish Browser pooling and the combined detailed Debug
...
3. **Browser encoded-input API.** RTCEncodedSource is a future simplification
...
4. **Broader quality work.** Reopen from measured benefit at acceptable complexity.
...
5. **Startup codec budget.** Compare bounded Auto and first-frame behavior on
...
6. **NAT field evidence.** Preserve [ADR-0009](./adr/0009-optional-nat-prediction.md).
...
7. **Control/resource fairness and input review.** Reassess the authenticated
...
8. **Reachable ownership/refactor work.** Extract page media-session owners only
...
9. **Storage fault recovery.** Ordinary transactions persist before memory
...
10. **Platform output.** System/tab mirroring needs no adapter. Remote Playback,
...
11. **Public presentation and piik rename.** The owner chose the future product
    name piik; final casing and its integration with the TV mascot remain a design
    review. Keep the two i letters readable while exploring eyes/antenna/feet.
    Prepare short, friendly English/Chinese README and Quick Start content;
    detailed implementation and deployment reference belong in developer docs.
    Draft a static GitHub Pages homepage with tutorials and self-contained visual
    demonstrations matching the living-room UI. The private production service
    is not a public demo and must not be advertised as one. Inventory names,
    package/module/config identifiers and assets before one coordinated rename;
    do not perform it during the Debug supplement or preserve old private aliases.
