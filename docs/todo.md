...
1. **Classify H.264 and select one fixed Browser codec.** Reproduce the dated blur/performance report on the same Host, browser, driver, game scene, and wired direct Viewer while changing one variable at a time. Choose VP8 or H.264 from fresh direct-share codec/profile/encoder and capture/sender/Viewer evidence, and make that codec the only Browser product choice. Validate browser relay and SFU only if the direct comparison selects H.264.
