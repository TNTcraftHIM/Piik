...
1. **Implement native SFU representation adaptation.** Publish fixed VP8 `HIGH+LOW` simulcast, enable Dynacast and LiveKit server send-side BWE, and keep AdaptiveStream disabled for current Viewers because any may become a relay. Preserve LiveKit-owned per-subscriber selection and the existing Host quality profiles; do not add a Screener quality controller or fall back to single HIGH for Host resource protection.
