...
1. **Deploy and physically prove Browser SFU ICE-server isolation.** Source implementation `ae09c760adec76fd26da611d4928486d105c6d3b` and its focused/full automated gates are complete. Deploy one immutable application artifact without changing LiveKit, coturn, nginx, systemd, firewall, ports, route deadline, or retry behavior. Close the P1 hold only after exact Host publication plus Viewer first-frame media passes under the reproducing TUN path and a separate mobile network; do not gate on candidate type/count or restore TURN.
