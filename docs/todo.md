...
1. **Deploy and physically validate the no-video-hint source.** Current source leaves video `contentHint` unset and retains audio `contentHint = "music"`; production still sets video `motion`. Deploy the source-only v10 change, then confirm real `getDisplayMedia()` capture and outbound stats on the production path. This does not decide the final codec.
...
| Final Browser video codec | Current v10 defaults to VP8 and retains the pre-share diagnostic selector. Compare no-hint VP8 and available hardware paths under real game load, then choose one codec and remove the selector, quality-state field, and share-lifetime codec branches atomically. Do not add application codec switching, GPU selection, SDP munging, or a codec ladder. |
