...
1. **Deploy Browser v12 only after explicit owner instruction.** Current source contains the strict v12 grant/access/UI and exact-audio-stat changes, including the two-state room-code contract; production remains exact `c4962f5` on v11. A release must switch server and Browser assets atomically, reject stale v11 before room authority, run bounded health/postflight checks, and verify the shorter complete invitation URL, rotate/revoke, public/password code entry, code-only missing-room result, direct media, and SFU fallback. This is an application-only release unless its scope is explicitly expanded.
