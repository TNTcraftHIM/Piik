...
1. **Implement, deploy, and physically validate fixed Browser VP8 with no video hint.** Replace the current v10 selector/state with one strict v11 wire: direct and browser-relay offers keep VP8 as the only media codec, SFU publication uses VP8 with no backup codec, video leaves `contentHint` unset, and audio keeps `contentHint = "music"`. Validate the source first, then deploy server/assets atomically and confirm actual codec/capture/outbound/decoded stats on direct, browser-relay, and SFU paths. Production remains v10 with video `motion` until that cutover.
