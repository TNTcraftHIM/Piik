...
1. **Route Wave D: one reconciliation loop.** Make the room controller the only committed graph owner and keep topology helpers to participant metadata plus pure deterministic candidate planning. Join, a hard-failed or non-paused decoded-frame-stalled edge, relay disconnect, and effective-capacity reduction all enqueue the same room-serial child operation. That one object owns a deterministic candidate list/cursor, one current candidate and its reservations, and one total deadline. Retain a usable old edge and commit only after the exact candidate child decodes its first new video frame; candidate failure advances the cursor, while operation exhaustion or deadline reaches explicit wait or failure. A relay with a bad ingress reparents itself and retains its subtree. Excess children use the same operation, and a disconnected endpoint is removed when childless. SFU-fed and peer-fed parents use this same first-child path; one Host publication serves all SFU subscriptions, and TURN changes only authorized edge transport. Authoritative pause aborts the operation and suppresses stall decisions until resume. Keep routing event-driven and edge-local; do not add proactive scoring, probing, rebalancing, or another graph/controller.
...
7. **Validate and release in priority order.** Review, merge, and deploy each coherent slice from canonical `main`: route first, then audio/codec mutation, then H.264/NAT evidence and any proven repair, then connection feedback and remaining tools. Before the route release, exercise `C=1/2/3`, effective capacity `0..C`, first-frame commit and rollback, stale ready, candidate cursor/total deadline, relay ingress reparent, disconnect/cap drain, SFU-fed first child, selected TURN, and pause/resume through focused contract tests plus one 20-Viewer browser smoke. Record route depth, first-frame time, endpoint sender counts, server CPU/memory and SFU/TURN usage; keep the one-operation controller event-driven and free of all-pairs probes. Run independent review, builds, production preflight, postflight, and rollback verification. A parallel branch may finish early but does not jump this rollout order.
