...
1. **Restore reliable Browser SFU initial ICE.** Apply `rtcConfig: { iceServers: [] }` to both pinned LiveKit publisher and subscriber connects so each SFU PC retains only LiveKit-signaled remote candidates as standard ICE inputs. Keep ordinary peer STUN and LiveKit server-side public-IP discovery unchanged; do not restore TURN, add a port, constrain candidate type/count, extend the route deadline, or add another recovery stage. Add focused connect-option tests, deploy one immutable application artifact, and close the P1 hold only after exact Host publication plus Viewer first-frame media passes under the reproducing TUN path and a separate mobile network.
