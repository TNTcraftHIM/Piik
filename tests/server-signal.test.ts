...
async function startPeerQualityReselectionFixture(
  prefix: string,
  now: () => number,
) {
  ...
  const viewerAuth = peerAssisted(
    await authenticate(viewer, harness.room, "viewer", `${prefix}-viewer`),
  );
  ...
  const connectionId = `${prefix}_connection`;
  host.socket.send(
    JSON.stringify({
      type: "signal",
...
describe("WebSocket signaling", () => {
  ...
  it("requires hard parent proof before low-FPS windows reparent", async () => {
    let now = 90_000;
    const fixture = await startPeerQualityReselectionFixture(
      "quality_low_fps_proof",
      () => now,
    );
    let sequence = 0;
    const sendWindow = async (
      ...
      fixture.viewer.socket.send(
        JSON.stringify(
          viewerQualityEvidenceWithMetrics(
            fixture.connectionId,
            fixture.viewerAuth.routeRevision,
            sequence,
            { framesPerSecond },
          ),
        ),
      ...
      sequence += 1;
      await correlateParentEdgeQualityEvidence(fixture.host, proof);
      now += 2_000;
    };

    for (let index = 0; index < 3; index += 1) {
      await sendWindow(40, { kind: "sending", packetsSentDelta: 1_500 });
    }
    await expect(
      fixture.viewer.inbox.next("route-update", 40),
    ).rejects.toThrow("Timed out");
    ...
    expect(moved.assignment.upstream).toEqual({
      kind: "peer",
      peerId: fixture.alternateParentAuth.peerId,
    });
  });

  it("resets a low-FPS streak at five-sixths of maxFramerate", async () => {
    let now = 120_000;
    const fixture = await startPeerQualityReselectionFixture(
      "quality_low_fps_reset",
      () => now,
    );
    let sequence = 0;
    const sendWindow = async (framesPerSecond: number) => {
      fixture.viewer.socket.send(
        JSON.stringify(
          viewerQualityEvidenceWithMetrics(
            fixture.connectionId,
  ...
  it("uses the current room maxFramerate for each low-FPS window", async () => {
    let now = 150_000;
    const fixture = await startPeerQualityReselectionFixture(
      "quality_low_fps_profile",
      () => now,
    );
    let sequence = 0;
    const sendWindow = async (framesPerSecond: number) => {
    ...
    expect(await fixture.viewer.inbox.next("quality-settings")).toEqual({
      type: "quality-settings",
      qualitySettings: balancedQualitySettings,
    });
