...
describe("DecodedFrameStallDetector", () => {
  it("reports one exact active stall and resets on progress or identity change", () => {
    ...
    expect(detector.observe("peer:a", 0, 0)).toBe(false);
    expect(
      detector.observe("peer:a", 0, ROUTE_DECODED_FRAME_STALL_MS),
    ).toBe(true);
    expect(
      detector.observe("peer:a", 0, ROUTE_DECODED_FRAME_STALL_MS + 1),
    ).toBe(false);
    expect(detector.observe("peer:a", 1, ROUTE_DECODED_FRAME_STALL_MS + 2)).toBe(
      false,
    );
    expect(
      detector.observe("peer:a", 0, ROUTE_DECODED_FRAME_STALL_MS * 2 + 2),
    ).toBe(true);
    expect(detector.observe("sfu:b", 0, ROUTE_DECODED_FRAME_STALL_MS * 3)).toBe(
      false,
    );
  });

  it("does not turn unknown samples into elapsed stall evidence", () => {
    const detector = new DecodedFrameStallDetector();
    expect(detector.observe("peer:a", 0, 0)).toBe(false);
    expect(
      detector.observe("peer:a", null, ROUTE_DECODED_FRAME_STALL_MS),
    ).toBe(false);
    expect(
      detector.observe(
        "peer:a",
        0,
        ROUTE_DECODED_FRAME_STALL_MS * 2 - 1,
      ),
    ).toBe(false);
    expect(
      detector.observe("peer:a", 0, ROUTE_DECODED_FRAME_STALL_MS * 2),
    ).toBe(true);
  });
  ...
  it("rebases elapsed time after the page resumes", () => {
    ...
    expect(
      detector.observe(
        "peer:1:a",
        0,
        ROUTE_DECODED_FRAME_STALL_MS * 6 - 1,
      ),
    ).toBe(true);
  });
