...
async function startHost(
  ...
  await waitForPage(
    cdp,
    page,
    "document.querySelector('.lr-entry-actions button.lr-tv-big.is-action[aria-label]')",
    15_000,
    "host controls",
    signal,
  );
  ...
  await evaluate(
    ...
    page,
    `(() => {
      const profiles = document.querySelectorAll('.lr-tiles button.lr-tile[aria-pressed]');
