...
if ($parent -ine $temp -or $name -notmatch '^screener-(native-one-viewer|access-privacy|client-loopback)-[A-Za-z0-9_-]{6}$') { exit 31 }
...
export function isExactGateProfile(
  ...
  return same &&
    /^screener-(?:native-one-viewer|access-privacy|client-loopback)-[A-Za-z0-9_-]{6}$/.test(
      basename(candidate),
    );
}
