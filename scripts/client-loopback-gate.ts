      ...
      const discover = async () => {
        for (let port = expected.portStart; port <= expected.portEnd; port += 1) {
          ...
          try {
            ...
            if (health.protocol === 1 && health.service === "screener-client" &&
      ...
      discover()
        .then((endpoint) => {
          ...
          const socket = new WebSocket(
            ...
            ["screener-client-v1." + endpoint.instanceToken],
          );
          let settled = false;
          const settle = () => {
            if (settled) return;
          ...
          socket.onopen = () => {
            const expectedProtocol = "screener-client-v1." + endpoint.instanceToken;
            ...
            socket.send(JSON.stringify({ version: 1, id: "request_hello", type: "hello" }));
          };
          ...
          socket.onmessage = (event) => {
            ...
            if (message.type === "ready") {
              try { socket.send(JSON.stringify({ version: 1, id: "request_ping", type: "ping" })); } catch (error) { result.error = String(error?.message || error); settle(); }
