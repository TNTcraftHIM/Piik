...
function temporaryRoot(prefix) {
  ...
  if (relation === "" || relation.startsWith("..") || resolve(parent, relation) !== resolved) {
    throw new Error("Client check created an unsafe temporary path");
  }
  return resolved;
}
...
function removeTemporaryRoot(path) {
  ...
  if (relation === "" || relation.startsWith("..") || resolve(parent, relation) !== path) {
    throw new Error("Client check refused to remove an unsafe path");
  }
...
function checkWindowsCapture() {
  ...
  const buildRoot = temporaryRoot("screener-capture-check-");
  try {
